...
class Wallet {
    ...
    sendSignedTxnandWaitConfirmation(rawTxn, numConfirmations) {
        return new Promise( (resolve, reject) => {
            var txn = this.web3.eth.sendSignedTransaction(rawTxn);

            txn.on('confirmation', (confirmationNumber, receipt) => {
              console.log('confirmation: ' + confirmationNumber);
              if (numConfirmations == confirmationNumber) {
