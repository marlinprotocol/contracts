    ...
    IClusterRewards // interface
{
    ...
    function feed(
        ...
        require(
            receiverStaking.START_TIME() + SWITCHING_PERIOD + 1 days >
                block.timestamp,
            "CRW:F-Invalid method"
        );
        uint256 rewardDistributed = rewardDistributedPerEpoch[_epoch];
        if (rewardDistributed == 0) {
            require(
                block.timestamp >
                    latestNewEpochRewardAt + rewardDistributionWaitTime,
        ...
        uint256 currentTotalRewardsPerEpoch = (((totalRewardsPerEpoch *
            1 days) / receiverStaking.EPOCH_LENGTH()) * networkRewardWeight) /
            totalRewardWeight;
        for (uint256 i = 0; i < _clusters.length; i++) {
            uint256 clusterReward = (currentTotalRewardsPerEpoch *
                _payouts[i]) / currentPayoutDenomination;
            rewardDistributed = rewardDistributed + clusterReward;
            clusterRewards[_clusters[i]] =
                clusterRewards[_clusters[i]] +
                clusterReward;
        }
        require(
    ...
    function _processReceiverTickets(
        ...
        unchecked {
            ...
            uint256 _rewardShare = (_totalNetworkRewardsPerEpoch *
                _epochReceiverStake) / _epochTotalStake;
            ...
            clusterRewards[_selectedClusters[i]] +=
                (_rewardShare *
                    uint256(RECEIVER_TICKETS_PER_EPOCH - _totalTickets)) /
                RECEIVER_TICKETS_PER_EPOCH;
        }
    ...
    function issueTickets(
        ...
        unchecked {
            for (uint256 i = 0; i < numberOfEpochs; ++i) {
                uint256 _totalNetworkRewardsPerEpoch = getRewardForEpoch(
                    _epochs[i],
                    _networkId
                );
                (
                ...
                ) = receiverStaking.getEpochInfo(_epochs[i]);
                require(
                ...
                (
                    uint256 _epochReceiverStake,
                    address _receiver
                ) = receiverStaking.balanceOfSignerAt(msg.sender, _epochs[i]);
    ...
    function issueTickets(bytes calldata _ticketInfo) external {
        ...
        unchecked {
            for (uint256 i = 0; i < _noOfEpochs; ++i) {
                _totalNetworkRewardsPerEpoch = _getRewardForEpoch(
                    _fromEpoch,
                    _networkId,
                    _epochLength
                );
