    ...
    IClusterRewards // interface
{
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < _networkIds.length; i++) {
            ...
            require(
                _clusterSelectors[i] != address(0),
                "CRW:CN-ClusterSelector must exist"
            );
            clusterSelectors[_networkIds[i]] = IClusterSelector(
                _clusterSelectors[i]
            );
    ...
    function addNetwork(
        ...
        address _clusterSelector
    ) external override onlyAdmin {
        require(rewardWeight[_networkId] == 0, "CRW:AN-Network already exists");
        require(
            _clusterSelector != address(0),
            "CRW:AN-ClusterSelector must exist"
        );
        rewardWeight[_networkId] = _rewardWeight;
        IClusterSelector networkClusterSelector = IClusterSelector(
            _clusterSelector
        );
        require(
            networkClusterSelector.START_TIME() == receiverStaking.START_TIME(),
            "CRW:AN-start time inconsistent"
        );
        require(
            networkClusterSelector.EPOCH_LENGTH() ==
                receiverStaking.EPOCH_LENGTH(),
    ...
    function removeNetwork(bytes32 _networkId) external override onlyAdmin {
        ...
        require(
            address(clusterSelectors[_networkId]) != address(0),
            "CRW:RN-Network doesnt exist"
        );
    ...
    function updateNetwork(
        bytes32 _networkId,
        uint256 _updatedRewardWeight,
        address _updatedClusterSelector
    ) external override onlyAdmin {
        uint256 networkWeight = rewardWeight[_networkId];
        require(
            _updatedClusterSelector != address(0),
            "CRW:UN-ClusterSelector must exist"
        );
        address currentClusterSelector = address(clusterSelectors[_networkId]);
        require(
            currentClusterSelector != address(0),
            "CRW:UN-Network doesnt exist"
        );

        if (_updatedClusterSelector != currentClusterSelector) {
            IClusterSelector networkClusterSelector = IClusterSelector(
                _updatedClusterSelector
            );
            require(
                networkClusterSelector.START_TIME() ==
                    receiverStaking.START_TIME(),
            ...
            clusterSelectors[_networkId] = IClusterSelector(
                _updatedClusterSelector
            );
        }
    ...
    function updateReceiverStaking(
        address _receiverStaking
    ) external onlyAdmin {
        _updateReceiverStaking(_receiverStaking);
    }
    ...
    function changeRewardPerEpoch(
        uint256 _updatedRewardPerEpoch
    ) external override onlyAdmin {
    ...
    function _getRewardShare(
        ...
        unchecked {
            ...
            _rewardShare =
                (_totalNetworkRewardsPerEpoch * _epochReceiverStake) /
                _epochTotalStake;
        }
    }
    ...
    function _processReceiverTickets(
        ...
        require(
            !_isTicketsIssued(_receiver, _epoch),
            "CRW:IPRT-Tickets already issued"
        );
        unchecked {
            require(
                _selectedClusters.length <= _tickets.length + 1,
                "CRW:IPRT-Tickets length not matching selected clusters"
            );
            ...
            for (; i < _selectedClusters.length - 1; ++i) {
                ...
                clusterRewards[_selectedClusters[i]] +=
                    (_rewardShare * uint256(_tickets[i])) /
            ...
            require(
                RECEIVER_TICKETS_PER_EPOCH >= _totalTickets,
    ...
    function _markAsIssued(address _receiver, uint256 _epoch) internal {
        unchecked {
            ...
            uint256 _pos = _epoch % 256;
            uint256 _issuedFlags = ticketsIssued[_receiver][_index];
            ticketsIssued[_receiver][_index] =
                _issuedFlags |
                (2 ** (255 - _pos));
        }
    }
    ...
    function issueTickets(
        ...
        unchecked {
            for (uint256 i = 0; i < _epochs.length; ++i) {
                _rewardToGive = MathUpgradeable.min(
                    receiverPayment.rewardRemaining,
                    receiverPayment.rewardPerEpoch
                );
                uint256 _epochTotalStake;
                {
                    uint256 _currentEpoch;
                    (_epochTotalStake, _currentEpoch) = receiverStaking
                        .getEpochInfo(_epochs[i]);

                    require(
                        _epochs[i] < _currentEpoch,
                        "CRW:IT-Epoch not completed"
                    );
                }
                address[] memory _selectedClusters = clusterSelectors[
                    _networkId
                ].getClusters(_epochs[i]);
                uint256 _epochReceiverStake = receiverStaking.balanceOfAt(
                    _receiver,
                    _epochs[i]
                );

                uint256 _rewardShare = _getRewardShare(
        ...
        receiverRewardPayment[_receiver].rewardRemaining = receiverPayment
            .rewardRemaining;
    }
    ...
    function issueTickets(bytes calldata _ticketInfo) external {
        ...
        require(
            _fromEpoch + _noOfEpochs <= _receiverStaking.getCurrentEpoch(),
            "CRW:ITC-Epochs not completed"
        );

        uint256[] memory _stakes = _receiverStaking.totalSupplyAtRanged(
            _fromEpoch,
            _noOfEpochs
        );
        (uint256[] memory _balances, address _receiver) = _receiverStaking
            .balanceOfSignerAtRanged(msg.sender, _fromEpoch, _noOfEpochs);
        address[][] memory _selectedClusters = clusterSelectors[_networkId]
            .getClustersRanged(_fromEpoch, _noOfEpochs);
        ...
        )
    {
        ...
        unchecked {
            ...
            uint256 clustersToSelect = clusterSelectors[networkId]
                .NUMBER_OF_CLUSTERS_TO_SELECT();
            // revert due to memory expansion overflow in case of underflow
            tickets[0] = new uint16[](clustersToSelect - 1);
            (
                uint256 _currentEpochIndex,
                uint256 _currentTicketIndex
            ) = _extractTickets(currentWord, 2, 0, 0, tickets);

            for (uint256 i = 1; i < noOfWords; ++i) {
                assembly {
                    currentWord := mload(
                        add(ticketInfo, add(0x40, mul(0x20, i)))
                    )
                }
    ...
    function _extractTickets(
        ...
        unchecked {
            for (uint256 i = startIndex; i < 16; ++i) {
                tickets[currentEpochIndex][currentTicketIndex] = uint16(
                    uint256(word >> (256 - (i + 1) * 16))
                );
                currentTicketIndex++;

                if (
                    currentTicketIndex > tickets[currentEpochIndex].length - 1
                ) {
                    if (currentEpochIndex == tickets.length - 1)
                        return (currentEpochIndex, currentTicketIndex);
                    currentEpochIndex++;
                    tickets[currentEpochIndex] = new uint16[](
                        tickets[currentEpochIndex - 1].length
                    );
    ...
    function issueTickets(
        ...
        (uint256 _epochTotalStake, uint256 _currentEpoch) = receiverStaking
            .getEpochInfo(_epoch);

        require(_epoch < _currentEpoch, "CRW:IT-Epoch not completed");

        (uint256 _epochReceiverStake, address _receiver) = receiverStaking
            .balanceOfSignerAt(msg.sender, _epoch);
        ReceiverPayment memory receiverPayment = receiverRewardPayment[
            _receiver
        ];
        ...
        address[] memory _selectedClusters = clusterSelectors[_networkId]
            .getClusters(_epoch);
        uint256 _totalNetworkRewardsPerEpoch = getRewardForEpoch(_networkId);
        uint256 _rewardToGive = MathUpgradeable.min(
            receiverPayment.rewardRemaining,
            receiverPayment.rewardPerEpoch
        );
    ...
    function claimReward(
        address _cluster
    ) external override onlyRole(CLAIMER_ROLE) returns (uint256) {
    ...
    function getRewardForEpoch(
        bytes32 _networkId
    ) public view override returns (uint256) {
        return
            (totalRewardsPerEpoch * rewardWeight[_networkId]) /
            totalRewardWeight;
    }

    //-------------------------------- User functions end --------------------------------//

    bytes32 public constant RECEIVER_PAYMENTS_MANAGER =
        keccak256("RECEIVER_PAYMENTS_MANAGER");

    mapping(address => ReceiverPayment) public receiverRewardPayment;

    function _increaseReceiverBalance(
        address staker,
        uint128 amount
    ) external override onlyRole(RECEIVER_PAYMENTS_MANAGER) {
        receiverRewardPayment[staker].rewardRemaining += amount;
    }

    function _setReceiverRewardPerEpoch(
        address staker,
