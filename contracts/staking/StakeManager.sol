    ...
    IStakeManager // interface
{
    ...
    bytes32 public constant DELEGATABLE_TOKEN_ROLE =
        keccak256("DELEGATABLE_TOKEN_ROLE");
    ...
    function _lockTokens(
        ...
        if (hasRole(DELEGATABLE_TOKEN_ROLE, tokenAddress)) {
            ...
            MPond(tokenAddress).delegate(_delegator, uint96(_amount));
        }
    }
    ...
    function _unlockTokens(
        ...
        if (hasRole(DELEGATABLE_TOKEN_ROLE, tokenAddress)) {
            ...
            MPond(tokenAddress).undelegate(_delegator, uint96(_amount));
        }
        IERC20Upgradeable(tokenAddress).safeTransfer(_delegator, _amount);
    }
    ...
    event StashMove(
        bytes32 indexed fromStashId,
        bytes32 indexed toStashId,
        bytes32[] tokenIds,
        uint256[] amounts
    );
    event StashDelegated(
        bytes32 indexed stashId,
        address indexed delegatedCluster
    );
    ...
    function _deposit(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(hasRole(ACTIVE_TOKEN_ROLE, tokens[_tokenId]));
            if (_amounts[i] != 0) {
                stashes[_stashId].amounts[_tokenId] =
                    stashes[_stashId].amounts[_tokenId] +
                    _amounts[i];
    ...
    function _move(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            bytes32 _tokenId = _tokenIds[i];
            stashes[_fromStashId].amounts[_tokenId] =
                stashes[_fromStashId].amounts[_tokenId] -
                _amounts[i];
            stashes[_toStashId].amounts[_tokenId] =
                stashes[_toStashId].amounts[_tokenId] +
                _amounts[i];
        }
    ...
    function _updateRewardDelegators(address _updatedRewardDelegator) internal {
        emit RewardDelegatorsUpdated(
            address(rewardDelegators),
            _updatedRewardDelegator
        );
    ...
    bytes32 public constant REDELEGATION_LOCK_SELECTOR =
        keccak256("REDELEGATION_LOCK");
    bytes32 public constant UNDELEGATION_LOCK_SELECTOR =
        keccak256("UNDELEGATION_LOCK");
    ...
    function delegateStash(
        bytes32 _stashId,
        address _delegatedCluster
    ) public onlyStakerOf(_stashId) {
        require(_delegatedCluster != address(0));
        require(stashes[_stashId].delegatedCluster == address(0));
        require(
            _lockStatus(UNDELEGATION_LOCK_SELECTOR, _stashId) !=
                LockStatus.Locked
        );
    ...
    function requestStashRedelegation(
        bytes32 _stashId,
        address _newCluster
    ) public onlyStakerOf(_stashId) {
        require(stashes[_stashId].delegatedCluster != address(0));
        require(_newCluster != address(0));
        _lock(
            REDELEGATION_LOCK_SELECTOR,
            _stashId,
            uint256(uint160(_newCluster))
        );
    }

    function requestStashRedelegations(
        bytes32[] memory _stashIds,
        address[] memory _newClusters
    ) public {
    ...
    function redelegateStash(bytes32 _stashId) public onlyStakerOf(_stashId) {
        require(stashes[_stashId].delegatedCluster != address(0));
        address _updatedCluster = address(
            uint160(_unlock(REDELEGATION_LOCK_SELECTOR, _stashId))
        );
        _redelegateStash(
            _stashId,
            stashes[_stashId].delegatedCluster,
            _updatedCluster
        );
    }
    ...
    function _cancelRedelegation(bytes32 _stashId) internal returns (bool) {
        bool _exists = _lockStatus(REDELEGATION_LOCK_SELECTOR, _stashId) !=
            LockStatus.None;
        if (_exists) {
    ...
    function splitStash(
        ...
        stashes[_newStashId].delegatedCluster = stashes[_stashId]
            .delegatedCluster;
    ...
    function mergeStash(
        bytes32 _stashId1,
        bytes32 _stashId2
    ) external onlyStakerOf(_stashId1) onlyStakerOf(_stashId2) {
        require(_stashId1 != _stashId2);
        require(
            stashes[_stashId1].delegatedCluster ==
                stashes[_stashId2].delegatedCluster
        );
        require(
            _lockStatus(UNDELEGATION_LOCK_SELECTOR, _stashId1) !=
                LockStatus.Locked &&
                _lockStatus(UNDELEGATION_LOCK_SELECTOR, _stashId2) !=
                LockStatus.Locked &&
                _lockStatus(REDELEGATION_LOCK_SELECTOR, _stashId1) ==
                LockStatus.None &&
                _lockStatus(REDELEGATION_LOCK_SELECTOR, _stashId2) ==
                LockStatus.None
        );
    ...
    function undelegateStash(bytes32 _stashId) public onlyStakerOf(_stashId) {
        ...
        _lock(
            UNDELEGATION_LOCK_SELECTOR,
            _stashId,
            uint256(uint160(_delegatedCluster))
        );
    ...
    function cancelUndelegation(
        ...
        address _delegatedCluster = address(
            uint160(_revertLock(UNDELEGATION_LOCK_SELECTOR, _stashId))
        );
