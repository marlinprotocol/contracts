...
contract RewardDelegators is Initializable, Ownable {
    ...
    modifier onlyStake() {
        require(
            msg.sender == stakeAddress,
            "ClusterRegistry:onlyStake: only stake contract can invoke this function"
        );
        _;
    }
    ...
    function _updateRewards(address _cluster) public {
        ...
        uint256 totalStakeAtReward = cluster.totalDelegation.pond.add(
            cluster.totalDelegation.mpond.mul(pondPerMpond)
        );
        ...
        uint256 commissionReward = reward
            .mul(clusterRegistry.getCommission(_cluster))
            .div(100);
        transferRewards(
            clusterRegistry.getRewardAddress(_cluster),
            commissionReward
        );
        uint256 delegatorReward = reward.sub(commissionReward);
        uint256 weightedStake = PondRewardFactor
            .mul(cluster.totalDelegation.pond)
            .add(
            MPondRewardFactor.mul(cluster.totalDelegation.mpond).mul(
                pondPerMpond
            )
        );
        cluster.accPondRewardPerShare = cluster.accPondRewardPerShare.add(
            delegatorReward.mul(PondRewardFactor).mul(10**30).div(weightedStake)
        );
        cluster.accMPondRewardPerShare = cluster.accMPondRewardPerShare.add(
            delegatorReward
    ...
    function delegate(
        ...
        uint256 delegatorEffectiveStake = delegatorStake.pond.add(
            delegatorStake.mpond.mul(pondPerMpond)
        );
        uint256 totalRewards = delegatorStake
            .pond
            .mul(clusterData.accPondRewardPerShare)
            .add(delegatorStake.mpond.mul(clusterData.accMPondRewardPerShare));
        if (
            delegatorEffectiveStake != 0 &&
            clusters[_cluster].lastDelegatorRewardDistNonce[_delegator] <
            currentNonce
        ) {
            uint256 pendingRewards = totalRewards.div(10**30).sub(
                clusters[_cluster].rewardDebt[_delegator]
            );
            if (pendingRewards != 0) {
        ...
        if (_PONDAmount != 0) {
            clusters[_cluster].totalDelegation.pond = clusterData
                .totalDelegation
                .pond
                .add(_PONDAmount);
            clusters[_cluster].delegators[_delegator].pond = delegatorStake
                .pond
                .add(_PONDAmount);
            totalRewards = totalRewards.add(
                _PONDAmount.mul(clusterData.accPondRewardPerShare)
            );
        }
        if (_MPONDAmount != 0) {
            clusters[_cluster].totalDelegation.mpond = clusterData
                .totalDelegation
                .mpond
                .add(_MPONDAmount);
            clusters[_cluster].delegators[_delegator].mpond = delegatorStake
                .mpond
                .add(_MPONDAmount);
            totalRewards = totalRewards.add(
                _MPONDAmount.mul(clusterData.accMPondRewardPerShare)
            );
        }
    ...
    function undelegate(
        ...
        if (_PONDAmount != 0) {
            clusters[_cluster].totalDelegation.pond = clusterData
                ...
                .pond
                .sub(_PONDAmount);
            clusters[_cluster].delegators[_delegator].pond = clusters[_cluster]
        ...
        if (_MPONDAmount != 0) {
            clusters[_cluster].totalDelegation.mpond = clusterData
                ...
                .mpond
                .sub(_MPONDAmount);
            clusters[_cluster].delegators[_delegator].mpond = clusters[_cluster]
                .delegators[_delegator]
                .mpond
                .sub(_MPONDAmount);
            totalRewards = totalRewards.sub(
    ...
    function withdrawRewards(address _delegator, address _cluster)
        public
        returns (uint256)
    {
        _updateRewards(_cluster);
        ...
        if (
            ...
            if (pendingRewards != 0) {
                ...
                clusters[_cluster]
                    .lastDelegatorRewardDistNonce[_delegator] = currentNonce;
                clusters[_cluster].rewardDebt[_delegator] = totalRewards.div(
                    10**30
                );
            }
    ...
    function getEffectiveStake(address _cluster) public view returns (uint256) {
        ...
        if (
            clusterRegistry.isClusterValid(_cluster) &&
            cluster.totalDelegation.mpond >= minMPONDStake
        ) {
            return (
                cluster.totalDelegation.pond.add(
                    cluster.totalDelegation.mpond.mul(pondPerMpond)
                )
            );
        }
    ...
    function updateUndelegationWaitTime(uint256 _undelegationWaitTime)
        public
        onlyOwner
    {
        undelegationWaitTime = _undelegationWaitTime;
