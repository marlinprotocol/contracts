...
//-------------------------------- Jobs start --------------------------------//
    ...
    IERC20 public token;
    uint256 public constant extraDecimals = 12;
    ...
    function _jobSettle(bytes32 _job) internal {
        ...
        uint256 _amount = (_rate * _usageDuration + 10**extraDecimals - 1) / 10**extraDecimals;
    ...
    function _jobWithdraw(bytes32 _job, address _to, uint256 _amount) internal {
        ...
        // leftover adjustment
        uint256 _leftover = (jobs[_job].rate * lockWaitTime[RATE_LOCK_SELECTOR] + 10**extraDecimals - 1) / 10**extraDecimals;
