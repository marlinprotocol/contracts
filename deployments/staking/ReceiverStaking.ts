...
export async function deploy(
  ...
  let receiverStaking = await upgrades.deployProxy(ReceiverStaking, [admin, "Receiver POND", "rPOND"], {
    kind: "uups",
    constructorArgs: [startTime, epochLength, stakingToken],
  });
...
export async function upgrade(startTime?: number, epochLength?: number, stakingToken?: string) {
  ...
  await upgradeUtil("ReceiverStaking", "ReceiverStaking", [startTime, epochLength, stakingToken]);
}
...
export async function verify() {
  ...
  await run("verify:verify", {
    address: implAddress,
    constructorArguments: [chainConfig.startTime, chainConfig.epochLength, chainConfig.staking.receiver.token],
  });
