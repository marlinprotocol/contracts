...
contract("Reward Pot", async function (accounts) {    
    ...
    it("check initilization variables", async () => {
        assert(
            localConfig.firstEpochStartBlock == await PotInstance.firstEpochStartBlock(), 
    ...
    it("Governance decision implemented", async () => {
        ...
        await PotInstance.changeEthBlocksPerEpoch(
            142, 
            {
                from: accounts[appConfig.governanceProxyAccountIndex]
            }
        ...
        await truffleAssert.reverts(
            PotInstance.changeEthBlocksPerEpoch(
                172, 
                {
                    from: accounts[10]
                }
    ...
    it("check ticket claiming by verifier contracts", async () => {
        ...
        await truffleAssert.reverts(
            PotInstance.claimTicket(
                [appConfig.roleParams.producer.roleId, appConfig.roleParams.receiver.roleId],
        ...
        await PotInstance.claimTicket(
            [appConfig.roleParams.producer.roleId, appConfig.roleParams.producer.roleId],
        ...
        assert(
            await PotInstance.getRemainingClaims(5, appConfig.roleParams.producer.roleId) == 4
        );
        assert(
            await PotInstance.getMaxClaims(5, appConfig.roleParams.producer.roleId) == 4
        );
    });
    ...
    it("check fee reward claims by winners", async () => {
        ...
        await truffleAssert.reverts(
            PotInstance.changeEpochsToWaitForClaims(7, originalEpoch, appConfig.roleParams.receiver.roleId, 
                {
                    from: accounts[appConfig.governanceProxyAccountIndex]
                }
            ), 
        ...
        await PotInstance.changeEpochsToWaitForClaims(7, originalEpoch+2, appConfig.roleParams.receiver.roleId, 
            {
                from: accounts[appConfig.governanceProxyAccountIndex]
            }
        );
        await PotInstance.addToPot([originalEpoch], accounts[0], appConfig.LINData.id, [20], 
            {
                from: accounts[20]
            }
        );
        ...
        await truffleAssert.reverts(
            PotInstance.claimFeeReward(
                appConfig.roleParams.receiver.roleId, [originalEpoch]
            ),
        ...
        currentEpoch = await PotInstance.getEpoch(
            await web3.eth.getBlockNumber()
        );
        console.log(currentEpoch);
    });
