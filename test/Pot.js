...
contract("Reward Pot", async function (accounts) {    
    ...
    it("Initialize Pot", async () => {
        ...
        await PotInstance.initialize(governanceProxy, firstEpochStartBlock, EthBlocksPerEpoch, roles, distribution, [appConfig.LINData.id], [LINInstance.address], claimWaitEpochs);
        await truffleAssert.reverts(PotInstance.initialize(governanceProxy, firstEpochStartBlock, EthBlocksPerEpoch, roles, distribution, [appConfig.LINData.id], [LINInstance.address], claimWaitEpochs));
    })
    ...
    it("check initilization variables", async () => {
        ...
        assert(appConfig.EthBlockPerEpoch == await PotInstance.blocksPerEpoch(), "blocksPerEpoch wasn't set");
        ...
        let { epochsToWaitForClaims } = await PotInstance.claimWait(localConfig.roles[roleIndex]);
        ...
        assert(tokenAddress == LINInstance.address, `TokenAddress not set, for tokenId ${appConfig.LINData.id}, expected: ${LINInstance.address}, got ${tokenAddress}`)
    });
    ...
    it("Governance decision implemented", async () => {
        ...
        await PotInstance.changeEpochsToWaitForClaims(1217, 2, "0x0", {from: accounts[appConfig.governanceProxyAccountIndex]});
        let claimWait = await PotInstance.claimWait("0x0");
        assert(claimWait.nextEpochsToWaitForClaims == 1217 && claimWait.epochOfepochsToWaitForClaimsUpdate == 2, "Governance unable to change epochsToWaitForClaims");

        await truffleAssert.reverts(PotInstance.changeEpochsToWaitForClaims(1000, 3, "0x0", {from: accounts[10]}), "Pot: Function can only be invoked by Governance Enforcer");
        let claimWaitNonGov = await PotInstance.claimWait("0x0");
        assert(claimWaitNonGov.nextEpochsToWaitForClaims != 1000 && claimWaitNonGov.epochOfepochsToWaitForClaimsUpdate != 3, "Random account is able to change epochsToWaitForClaims");
        // changeEthBlocksPerEpoch
        await PotInstance.changeEthBlocksPerEpoch(142, {from: accounts[appConfig.governanceProxyAccountIndex]});
        assert(await PotInstance.blocksPerEpoch() == 142, "Governance unable to change EthBlocksPerEpoch");
        ...
        assert(await PotInstance.blocksPerEpoch() != 172, "Random account is able to change EthBlocksPerEpoch");
        // allocatePot
    ...
    it.skip("check adding funds to pot", async () => {
        ...
        await truffleAssert.reverts(PotInstance.addToPot([2,3,4], accounts[0], appConfig.LINData.id, [20, 32, 47], {from: accounts[appConfig.governanceProxyAccountIndex]}));
        assert(LINInstance.allowance(accounts[0], PotInstance.address) == 40, "Allowance not set correctly");
        await LINInstance.approve(PotInstance.address, 99);
        await PotInstance.addToPot([2,3,4], accounts[0], appConfig.LINData.id, [20, 32, 47], {from: accounts[20]});
    ...
    it("check  pot allocation", async () => {
        await truffleAssert.reverts(PotInstance.allocatePot(["0x0", "0x1", "0x2"], [0, 17, 83]));
        await PotInstance.allocatePot(["0x0", "0x1", "0x2"], [0, 17, 83], {from: accounts[appConfig.governanceProxyAccountIndex]});
    ...
    it("check ticket claiming by verifier contracts", async () => {
        await PotInstance.addVerifier(accounts[10], {from: accounts[appConfig.governanceProxyAccountIndex]});
        await truffleAssert.reverts(PotInstance.claimTicket([appConfig.roleParams.producer.roleId, appConfig.roleParams.receiver.roleId],
            [accounts[12], accounts[13]], [5, 7, 9]));
        await truffleAssert.reverts(PotInstance.claimTicket([appConfig.roleParams.producer.roleId, appConfig.roleParams.receiver.roleId],
            [accounts[12], accounts[13]], [5, 7, 9], {from: accounts[10]}));
        await PotInstance.claimTicket([appConfig.roleParams.producer.roleId, appConfig.roleParams.receiver.roleId],
            [accounts[12], accounts[13]], [5, 7], {from: accounts[10]})
        assert(await PotInstance.getClaims(7, appConfig.roleParams.receiver.roleId, accounts[13]) == 1);
        assert(await PotInstance.getRemainingClaims(5, appConfig.roleParams.producer.roleId) == 1);
        assert(await PotInstance.getEpochAllocation(5, appConfig.roleParams.producer.roleId, appConfig.LINData.id) == 0);
        await PotInstance.claimTicket([appConfig.roleParams.producer.roleId, appConfig.roleParams.receiver.roleId],
            [accounts[12], accounts[13]], [5, 5], {from: accounts[10]});
        assert(await PotInstance.getClaims(5, appConfig.roleParams.receiver.roleId, accounts[13]) == 1);
        assert(await PotInstance.getClaims(5, appConfig.roleParams.producer.roleId, accounts[12]) == 2);
        assert(await PotInstance.getRemainingClaims(5, appConfig.roleParams.producer.roleId) == 2);
        await PotInstance.claimTicket([appConfig.roleParams.producer.roleId, appConfig.roleParams.producer.roleId],
            [accounts[14], accounts[13]], [5, 5], {from: accounts[10]});
        assert(await PotInstance.getRemainingClaims(5, appConfig.roleParams.producer.roleId) == 4);
        assert(await PotInstance.getMaxClaims(5, appConfig.roleParams.producer.roleId) == 4);
    });

    it("check fee reward claims by winners", async () => {
        await before();
        await PotInstance.changeEthBlocksPerEpoch(1, {from: accounts[appConfig.governanceProxyAccountIndex]});
        let verifierAccount = accounts[10];
        await PotInstance.addVerifier(verifierAccount, {from: accounts[appConfig.governanceProxyAccountIndex]});
        // approve LIN to pot
        await LINInstance.approve(PotInstance.address, 99);
        let originalEpoch = await PotInstance.getEpoch(await web3.eth.getBlockNumber());
        console.log(originalEpoch);
        await truffleAssert.reverts(PotInstance.changeEpochsToWaitForClaims(7, originalEpoch, appConfig.roleParams.receiver.roleId, {from: accounts[appConfig.governanceProxyAccountIndex]}), "Pot: can't  change wait time for claims in previous epochs");
        // this will be effected from next epoch
        await PotInstance.changeEpochsToWaitForClaims(7, originalEpoch+2, appConfig.roleParams.receiver.roleId, {from: accounts[appConfig.governanceProxyAccountIndex]});
        await PotInstance.addToPot([originalEpoch], accounts[0], appConfig.LINData.id, [20], {from: accounts[20]});
        await PotInstance.claimTicket([appConfig.roleParams.producer.roleId, appConfig.roleParams.receiver.roleId],
            [accounts[12], accounts[13]], [originalEpoch, originalEpoch], {from: accounts[10]});
        // Fails as wait time is not completed
        await truffleAssert.reverts(PotInstance.claimFeeReward(appConfig.roleParams.receiver.roleId, [originalEpoch]), "Pot: Fee can't be redeemed before wait time");
        let currentEpoch = await PotInstance.getEpoch(await web3.eth.getBlockNumber());
        console.log(currentEpoch);
        // 7 epochs passed at this epoch, so this is the edge case
        await truffleAssert.reverts(PotInstance.claimFeeReward(appConfig.roleParams.receiver.roleId, [originalEpoch], {from: accounts[13]}), "Pot: Fee can't be redeemed before wait time");
        // Should succeed as  wait time is done
        await PotInstance.claimFeeReward(appConfig.roleParams.receiver.roleId, [originalEpoch], {from: accounts[13]});
        // Should fail as fee is already
        await truffleAssert.reverts(PotInstance.claimFeeReward(appConfig.roleParams.receiver.roleId, [originalEpoch], {from: accounts[13]}), "Pot: No claims to redeem");
        // This fails because random accounts doesn't have a fee to claim.
        await truffleAssert.reverts(PotInstance.claimFeeReward(appConfig.roleParams.receiver.roleId, [originalEpoch]), "Pot: No claims to redeem");
