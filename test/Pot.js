...
contract.skip("Reward Pot", async function (accounts) {
  ...
  it("Deploy all contracts", async () => {
    await LINProxy.deployed().then(async (instance) => {
      LINInstance = await LINToken.at(instance.address);
      return LINInstance;
    });
    await PotProxy.deployed().then(async (instance) => {
  ...
  it("check initilization variables", async () => {
    ...
    assert(
      appConfig.EthBlockPerEpoch == (await PotInstance.EthBlocksPerEpoch()),
      "EthBlocksPerEpoch wasn't set"
    );
    ...
    let epochsToWaitForClaims = await PotInstance.epochsToWaitForClaims(
      localConfig.roles[roleIndex]
    );
    assert(
  ...
  it("Governance decision implemented", async () => {
    ...
    await truffleAssert.reverts(
      PotInstance.removeVerifier(tempVerfierAddress, {from: accounts[10]}),
    ...
    await PotInstance.changeEpochsToWaitForClaims(1217, "0x0", {
      from: accounts[appConfig.governanceProxyAccountIndex],
    });
    assert(
      (await PotInstance.epochsToWaitForClaims("0x0")) == 1217,
      "Governance unable to change epochsToWaitForClaims"
    );

    await truffleAssert.reverts(
      PotInstance.changeEpochsToWaitForClaims(1000, "0x0", {
        from: accounts[10],
      }),
      "Pot: Function can only be invoked by Governance Enforcer"
    );
    assert(
      (await PotInstance.epochsToWaitForClaims("0x0")) != 1000,
    ...
    assert(
      (await PotInstance.EthBlocksPerEpoch()) == 142,
      "Governance unable to change EthBlocksPerEpoch"
    );

    await truffleAssert.reverts(
      PotInstance.changeEthBlocksPerEpoch(172, {from: accounts[10]}),
      "Pot: Function can only be invoked by Governance Enforcer"
    );
    assert(
      (await PotInstance.EthBlocksPerEpoch()) != 172,
    ...
    await truffleAssert.reverts(
      PotInstance.allocatePot(["0x5", "0x6"], [61, 39], {from: accounts[10]}),
      "Pot: Function can only be invoked by Governance Enforcer"
    );
    assert(
