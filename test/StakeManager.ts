...
describe("StakeManager", function () {
  ...
  it("non admin cannot update RewardDelegatorsAddress", async () => {
    const RewardDelegators = await ethers.getContractFactory("RewardDelegators");
    let rewardDelegatorsTestInstance = await upgrades.deployProxy(RewardDelegators, {
      kind: "uups",
      initializer: false,
      constructorArgs: [pondTokenId, mpondTokenId],
    });
...
describe("StakeManager Deployment", function () {
  ...
  it("Create POND stash and split", async () => {
    ...
    let newStashTokenAmt = await stakeManagerInstance.getTokenAmountInStash(getNewStashId(splitTx.events), PONDTokenId);
    expect(newStashTokenAmt).equal(amount - 100);

    // old stash shouhld have 100
    let oldStashTokenAmt = await stakeManagerInstance.getTokenAmountInStash(getStashId(createStashTx.events), PONDTokenId);
  ...
  it("Create two stashes and then merge them", async () => {
    ...
    const mergedAmount = await stakeManagerInstance.getTokenAmountInStash(getStashId(createStash1.events), PONDTokenId);
    expect(mergedAmount).equal(10 * amount);

    // check if old stash has nothing
    const oldAmount = await stakeManagerInstance.getTokenAmountInStash(getStashId(createStash2.events), PONDTokenId);
