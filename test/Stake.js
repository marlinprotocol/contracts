...
contract.skip("Stake contract", async function (accounts) {
  ...
  it("create POND stash", async () => {
    ...
    assert(
      (await PONDInstance.allowance(accounts[0], stakeContract.address)) ==
        amount
    );
  ...
  it("create MPOND stash", async () => {
    ...
    assert(
      (await MPONDInstance.allowance(accounts[0], stakeContract.address)) ==
        amount
    );
  ...
  it("Delegate POND stash", async () => {
    ...
    const clusterInitialDelegation = await rewardDelegators.getClusterDelegation(
      registeredCluster
    );
    ...
    const clusterDelegation = await rewardDelegators.getClusterDelegation(
      registeredCluster
    );
  ...
  it("Delegate MPOND stash", async () => {
    ...
    await truffleAssert.reverts(
      clusterRegistry.register(5, registeredClusterRewardAddress, clientKey, {
        from: registeredCluster,
      })
    );
    const clusterInitialDelegation = await rewardDelegators.getClusterDelegation(
  ...
  it("Delegate POND to invalid cluster", async () => {
    ...
    const stashId = await createStash(0, amount);
    await truffleAssert.reverts(
      stakeContract.delegateStash(stashId, unregisteredCluster)
    );
  });
  ...
  it("Delegate MPOND to deregistered cluster", async () => {
    await clusterRegistry.register(
      5,
      deregisteredClusterRewardAddress,
      clientKey,
      {
    ...
    await truffleAssert.reverts(
      stakeContract.delegateStash(stashId, deregisteredCluster)
    );
  });
  ...
  it("Undelegate POND stash", async () => {
    ...
    const clusterDelegationAfterUndelegation = await rewardDelegators.getClusterDelegation(
      registeredCluster
    );
    assert(
      clusterInitialDelegation.POND.toString() ==
        clusterDelegationAfterUndelegation.POND.toString()
    );
  });
  ...
  it("Undelegate MPOND stash", async () => {
    ...
    assert(
      clusterInitialDelegation.MPOND.toString() ==
        clusterDelegationAfterUndelegation.MPOND.toString()
    );
  });
  ...
  it("Withdraw POND after wait time", async () => {
    ...
    const balanceBefore = (
      await PONDInstance.balanceOf(accounts[0])
    ).toString();
