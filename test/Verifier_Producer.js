...
contract.skip("Producer Verifier", (accounts) => {
  ...
  it("deploy all contracts", async () => {
    ...
    let VerifierProxyInstance = await ProducerVerifierProxy.new(
    ...
    let ClusterRegistryProxyInstance = await ClusterRegistryProxy.new(
      ClusterRegistryDeployment.address
    );
    ClusterRegistryInstance = await ClusterRegistry.at(
      ClusterRegistryProxyInstance.address
    );

    let ProducerDeployment = await Producer.new();
    let ProducerProxyInstance = await ProducerProxy.new(
      ProducerDeployment.address
    );
    ...
    let defaultClusterProxyInstance = await defaultClusterProxy.new(
      defaultClusterDeployment.address
    );
    DefaultClusterInstance = await defaultCluster.at(
      defaultClusterProxyInstance.address
    );
  });
