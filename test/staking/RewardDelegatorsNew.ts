...
describe("RewardDelegators init and upgrades", function () {
  ...
  it("deploys as proxy and initializes", async function () {
    ...
    const rewardDelegatorsUntyped: Contract = await upgrades.deployProxy(
      RewardDelegators,
      {
        kind: "uups",
        initializer: false,
    ...
    const rewardDelegators: RewardDelegators = getRewardDelegators(
      rewardDelegatorsUntyped.address,
      signers[0]
    );
    ...
    await initializationTx.to
      .emit(rewardDelegators, "AddReward")
      .withArgs(pondTokenId, stakingConfig.PondRewardFactor);
    await initializationTx.to
      .emit(rewardDelegators, "AddReward")
      .withArgs(mpondTokenId, stakingConfig.MPondRewardFactor);
    await initializationTx.to
    ...
    expect(await rewardDelegators.stakeAddress()).to.equal(
      fakeStakeManagerAddr
    );
    expect(await rewardDelegators.clusterRegistry()).to.equal(
      fakeClusterRegistryAddr
    );
    expect(await rewardDelegators.clusterRewards()).to.equal(
      fakeClusterRewardsAddr
    );
    expect(await rewardDelegators.PONDToken()).to.equal(fakePond);
    expect(await rewardDelegators.rewardFactor(pondTokenId)).to.equal(
      stakingConfig.PondRewardFactor
    );
    expect(await rewardDelegators.rewardFactor(mpondTokenId)).to.equal(
      stakingConfig.MPondRewardFactor
    );
    const [pondWeightForDelegation, pondWeightForThreshold] =
      await rewardDelegators.tokenWeights(pondTokenId);
    expect([
      pondWeightForDelegation.toNumber(),
      pondWeightForThreshold.toNumber(),
    ]).to.eql([
      parseInt(stakingConfig.PondWeightForThreshold),
      parseInt(stakingConfig.PondWeightForDelegation),
    ]);
    const [mpondWeightForDelegation, mpondWeightForThreshold] =
      await rewardDelegators.tokenWeights(mpondTokenId);
    expect([
      mpondWeightForDelegation.toNumber(),
    ...
    expect([
      await rewardDelegators.tokenList(0),
      await rewardDelegators.tokenList(1),
    ]).to.eql([pondTokenId, mpondTokenId]);
    expect(
      await rewardDelegators.hasRole(
        await rewardDelegators.DEFAULT_ADMIN_ROLE(),
        addrs[0]
      )
    ).to.be.true;
  });
  ...
  it("upgrades", async function () {
    ...
    const originalImplemetationAddress =
      await upgrades.erc1967.getImplementationAddress(rewardDelegators.address);
    await upgrades.upgradeProxy(rewardDelegators, RewardDelegators, {
      kind: "uups",
    });
  ...
  it("does not upgrade without admin", async () => {
    ...
    await upgrades.upgradeProxy(rewardDelegators.address, RewardDelegators, {
      kind: "uups",
    });
    ...
    await expect(
      upgrades.upgradeProxy(
        ...
        RewardDelegators.connect(signers[1]),
        { kind: "uups" }
      )
    ).to.be.reverted;
  });
...
describe("RewardDelegators global var updates", function () {
  ...
  it("non owner cannot update PondAddress", async () => {
    ...
    let pondInstance2 = await upgrades.deployProxy(Pond, ["Marlin", "POND"], {
      kind: "uups",
    });
    await expect(
      rewardDelegators
        .connect(signers[1])
        .updatePONDAddress(pondInstance2.address)
    ).to.be.reverted;
  });

  it("cannot update PondAddress to 0", async () => {
    await expect(
      rewardDelegators.updatePONDAddress(ethers.constants.AddressZero)
    ).to.be.reverted;
  });
  ...
  it("owner can update PondAddress", async () => {
    ...
    await expect(
      rewardDelegators.updatePONDAddress(pondInstance2.address)
    ).to.emit(rewardDelegators, "PONDAddressUpdated");
  });

  it("non owner cannot update ClusterRegistry", async () => {
    const ClusterRegistry = await ethers.getContractFactory("ClusterRegistry");
    let clusterRegistryInstance2 = await upgrades.deployProxy(ClusterRegistry, {
      kind: "uups",
      initializer: false,
    });
    await expect(
      rewardDelegators
        .connect(signers[1])
        .updateClusterRegistry(clusterRegistryInstance2.address)
    ).to.be.reverted;
  });

  it("cannot update ClusterRegistry to 0", async () => {
    await expect(
      rewardDelegators.updateClusterRegistry(ethers.constants.AddressZero)
    ).to.be.reverted;
  });
  ...
  it("owner can update ClusterRegistry", async () => {
    ...
    await expect(
      await rewardDelegators.updateClusterRegistry(
        clusterRegistryInstance2.address
      )
    ).to.emit(rewardDelegators, "ClusterRegistryUpdated");
  });
  ...
  it("non owner cannot update ClusterRewards", async () => {
    ...
    await expect(
      rewardDelegators
        .connect(signers[1])
        .updateClusterRewards(clusterRewardsInstance2.address)
    ).to.be.reverted;
  });

  it("cannot update ClusterRewards to 0", async () => {
    await expect(
      rewardDelegators.updateClusterRewards(ethers.constants.AddressZero)
    ).to.be.reverted;
  });
  ...
  it("owner can update ClusterRewards", async () => {
    ...
    await expect(
      await rewardDelegators.updateClusterRewards(
        clusterRewardsInstance2.address
      )
    ).to.emit(rewardDelegators, "ClusterRewardsAddressUpdated");
  });

  it("non owner cannot update StakeManager", async () => {
    const StakeManager = await ethers.getContractFactory("StakeManager");
    let stakeManagerInstance2 = await upgrades.deployProxy(StakeManager, {
      kind: "uups",
      initializer: false,
    });
    await expect(
      rewardDelegators
        .connect(signers[1])
        .updateStakeAddress(stakeManagerInstance2.address)
    ).to.be.reverted;
  });

  it("cannot update StakeManager to 0", async () => {
    await expect(
      rewardDelegators.updateStakeAddress(ethers.constants.AddressZero)
    ).to.be.reverted;
  });
  ...
  it("owner can update StakeManager", async () => {
    ...
    await expect(
      await rewardDelegators.updateStakeAddress(stakeManagerInstance2.address)
    ).to.emit(rewardDelegators, "StakeAddressUpdated");
  });

  it("non owner cannot add rewardFactor", async () => {
    let testTokenId = await ethers.utils.keccak256(addrs[0]);
    await expect(
      rewardDelegators.connect(signers[1]).addRewardFactor(testTokenId, 1)
    ).to.be.reverted;
  });

  it(" cannot add for already existing tokenId", async () => {
    await expect(
      rewardDelegators.addRewardFactor(
        "" + pondTokenId,
        stakingConfig.PondRewardFactor
      )
    ).to.be.reverted;
  ...
  it("cannot add 0 reward Factor", async () => {
    let testTokenId = await ethers.utils.keccak256(addrs[0]);
    await expect(rewardDelegators.addRewardFactor(testTokenId, 0)).to.be
      .reverted;
  });
  ...
  it("owner can add rewardFactor", async () => {
    let testTokenId = await ethers.utils.keccak256(addrs[0]);
    await expect(
      await rewardDelegators.addRewardFactor(testTokenId, 1)
    ).to.emit(rewardDelegators, "AddReward");
  });

  it("non owner cannot remove rewardFactor", async () => {
    await expect(
      rewardDelegators.connect(signers[1]).removeRewardFactor("" + pondTokenId)
    ).to.be.reverted;
  });

  it("cannot remove non existing tokenId", async () => {
    let testTokenId = await ethers.utils.keccak256(addrs[0]);
    await expect(rewardDelegators.removeRewardFactor(testTokenId)).to.be
      .reverted;
  });

  it("owner can remove rewardFactor", async () => {
    await expect(
      await rewardDelegators.removeRewardFactor("" + pondTokenId)
    ).to.emit(rewardDelegators, "RemoveReward");
  });
  ...
  it("non owner cannot update reward Factor", async () => {
    await expect(
      rewardDelegators
        ...
        .updateRewardFactor(
          "" + pondTokenId,
          stakingConfig.PondRewardFactor + 1
        )
    ).to.be.reverted;
  });
  ...
  it("cannot update non existing tokenId", async () => {
    let testTokenId = await ethers.utils.keccak256(addrs[0]);
    await expect(rewardDelegators.updateRewardFactor(testTokenId, 1)).to.be
      .reverted;
  });

  it("cannot update rewardFactor to 0", async () => {
    await expect(rewardDelegators.updateRewardFactor("" + pondTokenId, 0)).to.be
      .reverted;
  });
  ...
  it("owner can update rewardFactor", async () => {
    ...
    ).to.emit(rewardDelegators, "RewardsUpdated");
  });
});
...
describe("RewardDelegators ", function () {
  ...
  before(async function () {
    ...
    fakeStakeManager = await deployMockContract(
      signers[9],
      StakeManager.interface.format()
    );
    imperonatedStakeManager = await impersonate(
      ethers,
    ...
    const ClusterRewards = await ethers.getContractFactory("ClusterRewards");
    fakeClusterRewards = await deployMockContract(
      signers[8],
      ClusterRewards.interface.format()
    );
    const ClusterRegistry = await ethers.getContractFactory("ClusterRegistry");
    fakeClusterRegistry = await deployMockContract(
      signers[7],
      ClusterRegistry.interface.format()
    );
    impersonatedClusterRegistry = await impersonate(
      ethers,
      fakeClusterRegistry.address
    );
    await setBalance(ethers, fakeClusterRegistry.address, e18);
    const Pond = await ethers.getContractFactory("Pond");
    const pondUntyped = await upgrades.deployProxy(Pond, ["Marlin", "POND"], {
      kind: "uups",
    });
    ...
    await fakeClusterRewards.mock.clusterSelectors.returns(
      ethers.constants.AddressZero
    );
    fakeClusterSelectors["ETH"] = await deployMockContract(
      signers[5],
      clusterSelector.interface.format()
    );
    await fakeClusterRewards.mock.clusterSelectors
      .withArgs(ethers.utils.id("ETH"))
      .returns(fakeClusterSelectors["ETH"].address);
    fakeClusterSelectors["DOT"] = await deployMockContract(
      signers[4],
      clusterSelector.interface.format()
    );
    await fakeClusterRewards.mock.clusterSelectors
      .withArgs(ethers.utils.id("DOT"))
      .returns(fakeClusterSelectors["DOT"].address);
  });
  ...
  it("delegate to cluster single token", async () => {
    ...
    const cluster = await registeredClusters[
      FuzzedNumber.randomInRange(0, registeredClusters.length).toNumber()
    ].getAddress();
    const rewardAddress =
      registeredClusterRewardAddresses[
        FuzzedNumber.randomInRange(
          0,
          registeredClusterRewardAddresses.length
        ).toNumber()
      ];
    ...
    await fakeClusterRewards.mock.claimReward
      .withArgs(cluster)
      .returns(rewardAmount);
    await fakeClusterRegistry.mock.getRewardInfo
      .withArgs(cluster)
      .returns(commission, rewardAddress);
    await fakeClusterRegistry.mock.getNetwork
      .withArgs(cluster)
      .returns(networkId);
    ...
    await expect(
      rewardDelegators.delegate(delegator, cluster, [pondTokenId], [amount1])
    ).to.be.revertedWith("RD:OS-only stake contract can invoke");

    let clusterDelegationInit = await rewardDelegators.getClusterDelegation(
      cluster,
      pondTokenId
    );
    let delegationInit = await rewardDelegators.getDelegation(
      cluster,
      delegator,
      pondTokenId
    );
    let rewardPerShare = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );
    await expect(
      rewardDelegators
        ...
        .delegate(delegator, cluster, [pondTokenId], [amount1])
    ).to.changeTokenBalances(
      pond,
      [rewardDelegators, rewardAddress, delegator],
      [
        -commission.mul(rewardAmount).div(100),
        commission.mul(rewardAmount).div(100),
        0,
      ]
    );
    let clusterDelegation = await rewardDelegators.getClusterDelegation(
      cluster,
      pondTokenId
    );
    let delegation = await rewardDelegators.getDelegation(
      cluster,
      delegator,
      pondTokenId
    );
    let rewardPerShare1 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );
    ...
    clusterDelegationInit = await rewardDelegators.getClusterDelegation(
      cluster,
      pondTokenId
    );
    delegationInit = await rewardDelegators.getDelegation(
      cluster,
      delegator1,
      pondTokenId
    );
    ...
    await expect(
      rewardDelegators
        .connect(imperonatedStakeManager)
        .delegate(delegator1, cluster, [pondTokenId], [amount1])
    ).to.changeTokenBalances(
      pond,
      [rewardDelegators, rewardAddress, delegator1],
      [-clusterCommission, clusterCommission, 0]
    );
    clusterDelegation = await rewardDelegators.getClusterDelegation(
      cluster,
      pondTokenId
    );
    delegation = await rewardDelegators.getDelegation(
      cluster,
      delegator1,
      pondTokenId
    );
    const rewardPerShare2 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );
    ...
    expect(delegation).to.equal(delegationInit.add(amount1));
    // as there is only pond token all reward  is given to pond token
    expect(rewardPerShare2.sub(rewardPerShare1)).to.equal(
      rewardAmount.sub(clusterCommission).mul(e30).div(clusterDelegationInit)
    );
    ...
    let delegatorCurrentReward = rewardPerShare2
      .sub(rewardPerShare)
      .mul(delegationInit)
      .div(e30);
    await expect(
    ...
    const rewardPerShare3 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );
    ...
    await fakeClusterRewards.mock.claimReward
      .withArgs(cluster)
      .returns(rewardAmount1);
    clusterCommission = rewardAmount1.mul(commission).div(100);
    await expect(
      rewardDelegators._updateRewards(cluster)
    ).to.changeTokenBalances(
      pond,
      [rewardDelegators, rewardAddress, delegator, delegator1],
      [-clusterCommission, clusterCommission, 0, 0]
    );
    const rewardPerShare4 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );

    expect(rewardPerShare4.sub(rewardPerShare3)).to.equal(
      rewardAmount1.sub(clusterCommission).mul(e30).div(clusterDelegation)
    );
    ...
    let delegator1CurrentReward = rewardPerShare4
      .add(
        rewardAmount1.sub(clusterCommission).mul(e30).div(clusterDelegationInit)
      )
      .sub(rewardPerShare2)
      .mul(delegationInit)
      .div(e30)
    ...
    await expect(
      rewardDelegators
        .connect(imperonatedStakeManager)
        .delegate(delegator1, cluster, [pondTokenId], [amount2])
    ).to.changeTokenBalances(
      pond,
      [rewardDelegators, rewardAddress, delegator1],
      [
        -delegator1CurrentReward.add(clusterCommission),
        clusterCommission,
    ...
    const rewardPerShare5 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );
    ...
    expect(rewardPerShare5.sub(rewardPerShare4)).to.equal(
      rewardAmount1.sub(clusterCommission).mul(e30).div(clusterDelegationInit)
    );
    ...
    await expect(
      rewardDelegators
        .connect(imperonatedStakeManager)
        .delegate(delegator, cluster, [pondTokenId], [amount2])
    ).to.changeTokenBalances(
      pond,
      [rewardDelegators, rewardAddress, delegator],
      [-delegatorCurrentReward, 0, delegatorCurrentReward]
    ...
    const rewardPerShare6 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );
    ...
    expect(clusterDelegation).to.equal(clusterDelegationInit.add(amount2));
    expect(delegation).to.equal(delegationInit.add(amount2));
    expect(rewardPerShare6.sub(rewardPerShare5)).to.equal(
      rewardAmount1.mul(e30).div(clusterDelegationInit)
    );
  });
  ...
  it("delegate to cluster multiple tokens", async () => {
    ...
    let mpondClusterDelegationInit =
    ...
    let mpondDelegationInit = await rewardDelegators.getDelegation(
      cluster,
      delegator,
      mpondTokenId
    );
    ...
    let mpondRewardPerShare = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );
    await expect(
    ...
    let mpondClusterDelegation = await rewardDelegators.getClusterDelegation(
      cluster,
      mpondTokenId
    );
    let delegation = await rewardDelegators.getDelegation(
    ...
    let mpondDelegation = await rewardDelegators.getDelegation(
      cluster,
      delegator,
      mpondTokenId
    );
    ...
    let mpondRewardPerShare1 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );

    expect(clusterDelegation).to.equal(clusterDelegationInit.add(amount1));
    expect(mpondClusterDelegation).to.equal(
      mpondClusterDelegationInit.add(mpondAmount1)
    );
    expect(delegation).to.equal(delegationInit.add(amount1));
    ...
    mpondClusterDelegationInit = await rewardDelegators.getClusterDelegation(
      cluster,
      mpondTokenId
    );
    delegationInit = await rewardDelegators.getDelegation(
    ...
    mpondDelegationInit = await rewardDelegators.getDelegation(
      cluster,
      delegator1,
      mpondTokenId
    );
    rewardPerShare = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );
    mpondRewardPerShare = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );
    let clusterCommission = commission.mul(rewardAmount).div(100);
    ...
    mpondClusterDelegation = await rewardDelegators.getClusterDelegation(
      cluster,
      mpondTokenId
    );
    delegation = await rewardDelegators.getDelegation(
    ...
    mpondDelegation = await rewardDelegators.getDelegation(
      cluster,
      delegator1,
      mpondTokenId
    );
    ...
    const mpondRewardPerShare2 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );
    ...
    expect(mpondRewardPerShare2.sub(mpondRewardPerShare1)).to.equal(
      rewardAmount
        ...
        .mul(e30)
        .div(2)
        .div(mpondClusterDelegationInit)
    );
    ...
    let delegatorCurrentReward = rewardPerShare2
      ...
      .mul(delegationInit)
      .div(e30);
    let delegatorCurrentRewardMpond = mpondRewardPerShare2
      .sub(mpondRewardPerShare)
    ...
    const mpondRewardPerShare3 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );
    ...
    const mpondRewardPerShare4 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );
    ...
    await expect(
      ...
      [
        -delegator1CurrentReward
          .add(delegator1CurrentRewardMpond)
          .add(clusterCommission),
        clusterCommission,
    ...
    const mpondRewardPerShare5 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );
    ...
    expect(clusterDelegation).to.equal(clusterDelegationInit.add(amount2));
    expect(mpondClusterDelegation).to.equal(
      mpondClusterDelegationInit.add(mpondAmount2)
    );
    expect(delegation).to.equal(delegationInit.add(amount2));
    ...
    // TODO: Fix add(1) at the end
    let changeInRewardPerShare = rewardAmount1
      .mul(e30)
      .div(2)
      .div(clusterDelegationInit);
    let changeInRewardPerShareMpond = rewardAmount1
      .mul(e30)
      .div(2)
      .div(mpondClusterDelegation);
    delegatorCurrentReward = rewardPerShare5
      .add(changeInRewardPerShare)
      .sub(rewardPerShare3)
      .mul(delegationInit)
      .div(e30)
      .add(1);
    ...
    const mpondRewardPerShare6 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );
    ...
    expect(mpondDelegation).to.equal(mpondDelegationInit.add(mpondAmount2));
    expect(rewardPerShare6.sub(rewardPerShare5)).to.equal(
      changeInRewardPerShare
    );
    expect(mpondRewardPerShare6.sub(mpondRewardPerShare5)).to.equal(
      changeInRewardPerShareMpond
    );
  });
  ...
  it("undelegate from cluster", async () => {
    ...
    const rewardPerShare1 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );
    const mpondRewardPerShare1 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );
    await rewardDelegators
    ...
    let rewardPerShare3 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );
    let mpondRewardPerShare3 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );

    expect(clusterDelegation).to.equal(
      clusterDelegationInit.sub(amount1.div(3))
    );
    expect(mpondClusterDelegation).to.equal(
      mpondClusterDelegationInit.sub(mpondAmount1.div(3))
    );
    expect(delegation).to.equal(delegationInit.sub(amount1.div(3)));
    expect(mpondDelegation).to.equal(
      mpondDelegationInit.sub(mpondAmount1.div(3))
    );
    expect(rewardPerShare3.sub(rewardPerShare2)).to.equal(0);
    expect(mpondRewardPerShare3.sub(mpondRewardPerShare2)).to.equal(0);
    ...
    let changeInRewardPerShareMpond = rewardAmount1
      ...
      .div(2)
      .div(mpondClusterDelegation);
    delegatorCurrentReward = rewardPerShare4
      .add(changeInRewardPerShare)
      .sub(rewardPerShare2)
      .mul(delegationInit)
      .div(e30)
    ...
    let rewardPerShare5 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      pondTokenId
    );
    let mpondRewardPerShare5 = await rewardDelegators.getAccRewardPerShare(
      cluster,
      mpondTokenId
    );

    expect(clusterDelegation).to.equal(
      clusterDelegationInit.sub(amount2.div(2))
    );
    expect(mpondClusterDelegation).to.equal(
      mpondClusterDelegationInit.sub(mpondAmount2.div(2))
    );
    expect(delegation).to.equal(delegationInit.sub(amount2.div(2)));
    expect(mpondDelegation).to.equal(
      mpondDelegationInit.sub(mpondAmount2.div(2))
    );
    expect(rewardPerShare5.sub(rewardPerShare4)).to.equal(
      changeInRewardPerShare
    );
    expect(mpondRewardPerShare5.sub(mpondRewardPerShare4)).to.equal(
      changeInRewardPerShareMpond
    );
  ...
  it("update cluster delegation", async () => {
    ...
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator, cluster, [pondTokenId], [10000000000]);
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator, cluster1, [mpondTokenId], [6]);
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator, cluster2, [mpondTokenId], [10]);

    await fakeClusterSelectors["ETH"].mock.upsert.revertsWithReason(
      "unexpected upsert"
    );
    await fakeClusterSelectors["ETH"].mock.deleteIfPresent.revertsWithReason(
      "unexpected delete"
    );
    ...
    await expect(
      rewardDelegators
        .connect(impersonatedClusterRegistry)
        .updateClusterDelegation(cluster, networkId)
    ).to.be.revertedWith("unexpected delete");
    await expect(
      rewardDelegators
        .connect(impersonatedClusterRegistry)
        .updateClusterDelegation(cluster1, networkId)
    ).to.be.revertedWith("unexpected delete");
    await expect(
      rewardDelegators
        .connect(impersonatedClusterRegistry)
        .updateClusterDelegation(cluster2, networkId)
    ...
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator, cluster, [mpondTokenId], [12]);
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator, cluster1, [pondTokenId], [4000000]);
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .undelegate(delegator, cluster2, [mpondTokenId], [1]);
    ...
    await rewardDelegators
      .connect(impersonatedClusterRegistry)
      .updateClusterDelegation(cluster, ethers.utils.id("RANDOM"));
  ...
  it("remove cluster delegation", async () => {
    ...
    expect(fakeClusterSelectors["ETH"].address).to.not.equal(
      ethers.constants.AddressZero
    );
    await rewardDelegators
      .connect(impersonatedClusterRegistry)
      .removeClusterDelegation(cluster, ethers.utils.id("RANDOM"));
    await expect(
      rewardDelegators
        .connect(impersonatedClusterRegistry)
        .removeClusterDelegation(cluster, ethers.utils.id("ETH"))
    ).to.be.revertedWith("delete called");
  });
  ...
  it("withdraw rewards", async () => {
    ...
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator, cluster, [pondTokenId], [20000000]);
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator1, cluster1, [pondTokenId], [100000]);
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator1, cluster, [pondTokenId, mpondTokenId], [4000000, 6]);
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator1, cluster2, [mpondTokenId], [10]);
    ...
    let delegatorClusterReward = Math.floor(
      (rewardAfterCommission * 20000000) / (20000000 + 4000000) / 2
    );
    await expect(
      rewardDelegators["withdrawRewards(address,address)"](delegator, cluster)
    ).to.changeTokenBalances(
      pond,
    ...
    await expect(
      rewardDelegators["withdrawRewards(address,address)"](delegator1, cluster1)
    ).to.changeTokenBalances(
      pond,
    ...
    await expect(
      rewardDelegators["withdrawRewards(address,address)"](delegator1, cluster2)
    ).to.changeTokenBalances(
      pond,
    ...
    await expect(
      rewardDelegators["withdrawRewards(address,address)"](delegator1, cluster)
    ).to.changeTokenBalances(
      pond,
      [rewardDelegators.address, delegator1, cluster],
      [
        -(rewardAfterCommission - delegatorClusterReward),
        rewardAfterCommission - delegatorClusterReward,
        0,
      ],
      2
  ...
  it("refresh cluster delegation", async () => {
    ...
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator, cluster, [pondTokenId], [1000000]);
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator1, cluster1, [pondTokenId], [10000]);
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator1, cluster, [pondTokenId, mpondTokenId], [2000000, 6]);
    await rewardDelegators
      .connect(imperonatedStakeManager)
      .delegate(delegator1, cluster2, [mpondTokenId], [100]);

    await fakeClusterSelectors["ETH"].mock.upsertMultiple
      .withArgs([cluster, cluster1, cluster2], [3000, 100, 10000])
      .returns();
    await rewardDelegators.refreshClusterDelegation(networkId, [
      cluster,
      cluster1,
      cluster2,
    ]);
    ...
    await fakeClusterSelectors["ETH"].mock.upsertMultiple
      .withArgs([cluster, cluster2], [3000, 10000])
      .returns();
    await rewardDelegators.refreshClusterDelegation(networkId, [
      cluster,
      cluster1,
