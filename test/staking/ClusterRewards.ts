...
const tickets = [
  ...
  MAX_TICKETS.sub(
    MAX_TICKETS.mul(10).div(100).add(MAX_TICKETS.mul(20).div(100)).add(MAX_TICKETS.mul(30).div(100)).add(MAX_TICKETS.mul(15).div(100))
  ),
];
...
describe("ClusterRewards deploy and init", function () {
  ...
  it("upgrades", async function () {
    ...
    await upgrades.upgradeProxy(clusterRewards.address, ClusterRewards, { kind: "uups", constructorArgs: [] });
...
describe("ClusterRewards add network", function () {
  ...
  it("admin cannot add existing network", async function () {
    ...
    await expect(clusterRewards.addNetwork(ethers.utils.id("ETH"), 400, clusterSelector.address)).to.be.revertedWith(
      "CRW:AN-Network already exists"
    );
  });

  it("admin cannot add network with invalid cluster selector", async function () {
    await expect(clusterRewards.addNetwork(ethers.utils.id("POLYGON"), 400, ethers.constants.AddressZero)).to.be.revertedWith(
      "CRW:AN-ClusterSelector must exist"
    );
  });
  ...
  it("admin cannot add network if start time does not match", async function () {
    ...
    await expect(clusterRewards.addNetwork(ethers.utils.id("POLYGON"), 400, clusterSelector.address)).to.be.revertedWith(
      "CRW:AN-start time inconsistent"
    );
  });
  ...
  it("admin cannot add network if epoch length does not match", async function () {
    ...
    await expect(clusterRewards.addNetwork(ethers.utils.id("POLYGON"), 400, clusterSelector.address)).to.be.revertedWith(
      "CRW:AN-epoch length inconsistent"
    );
  });
  ...
  it("non admin cannot add network", async function () {
    ...
    await expect(
      clusterRewards.connect(signers[1]).addNetwork(ethers.utils.id("POLYGON"), 400, clusterSelector.address)
    ).to.be.revertedWith("only admin");
  });
});
...
describe("ClusterRewards cluster selector", function () {
  ...
  it("admin cannot update cluster selector to zero", async function () {
    await expect(clusterRewards.updateNetwork(ethers.utils.id("ETH"), ETHWEIGHT, ethers.constants.AddressZero)).to.be.revertedWith(
      "CRW:UN-ClusterSelector must exist"
    );
  });
  ...
  it("admin cannot update cluster selector if start time does not match", async function () {
    ...
    await expect(clusterRewards.updateNetwork(ethers.utils.id("ETH"), ETHWEIGHT, clusterSelector.address)).to.be.revertedWith(
      "CRW:UN-start time inconsistent"
    );
  });
  ...
  it("admin cannot update cluster selector if epoch length does not match", async function () {
    ...
    await expect(clusterRewards.updateNetwork(ethers.utils.id("ETH"), ETHWEIGHT, clusterSelector.address)).to.be.revertedWith(
      "CRW:UN-epoch length inconsistent"
    );
  });
  ...
  it("admin cannot update cluster selector and weight for non existent network", async function () {
    ...
    await expect(clusterRewards.updateNetwork(ethers.utils.id("POLYGON"), ETHWEIGHT, clusterSelector.address)).to.be.revertedWith(
      "CRW:UN-Network doesnt exist"
    );
  });
  ...
  it("non admin cannot update cluster selector", async function () {
    ...
    await expect(
      clusterRewards.connect(signers[1]).updateNetwork(ethers.utils.id("ETH"), ETHWEIGHT, clusterSelector.address)
    ).to.be.revertedWith("only admin");
  });
  ...
  it("non admin cannot update cluster selector and weight", async function () {
    ...
    await expect(clusterRewards.connect(signers[1]).updateNetwork(ethers.utils.id("ETH"), 400, clusterSelector.address)).to.be.revertedWith(
      "only admin"
    );
  });
});
...
describe("ClusterRewards feed rewards", function () {
  ...
  it("feeder cannot feed rewards after 1 day after switching time", async function () {
    ...
    await expect(
      clusterRewards.connect(signers[2]).feed(ETHHASH, [addrs[21], addrs[22]], [e16.mul(10), e16.mul(50)], 1)
  ...
  it("feeder cannot feed rewards exceeding total", async function () {
    await expect(
      clusterRewards
        .connect(signers[2])
        .feed(ETHHASH, [addrs[21], addrs[22], addrs[23]], [e16.mul(10), e16.mul(50), e16.mul(40).add(e16.div(100))], 1)
    ).to.be.revertedWith("CRW:F-Reward Distributed  cant  be more  than totalRewardPerEpoch");
  });
  ...
  it("feeder cannot feed rewards in multiple parts exceeding total", async function () {
    ...
    await expect(clusterRewards.connect(signers[2]).feed(ETHHASH, [addrs[23]], [e16.mul(41)], 1)).to.be.revertedWith(
  ...
  it("feeder cannot feed rewards again before wait time", async function () {
    ...
    await expect(
      clusterRewards.connect(signers[2]).feed(ETHHASH, [addrs[21], addrs[22]], [e16.mul(10), e16.mul(10)], 2)
    ).to.be.revertedWith("CRW:F-Cant distribute reward for new epoch within such short interval");
...
describe("ClusterRewards submit tickets", function () {
  ...
  it("staker cannot submit tickets multiple times for the same epoch", async function () {
    ...
    await expect(
      clusterRewards.connect(signers[5])["issueTickets(bytes32,uint24,uint16[])"](ETHHASH, epochWithRewards, tickets.slice(0, -1))
  ...
  it("staker can submit tickets with extra rewards enabled", async function () {
    ...
    await clusterRewards
      .connect(signers[5])
      ["issueTickets(bytes32,uint24[],uint16[][])"](ETHHASH, [epochWithRewards], [tickets.slice(0, -1)]);
...
describe("ClusterRewards submit compressed tickets", function () {
  ...
  it("staker can submit compressed tickets after switch with non zero rewards", async function () {
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      receiverRewards1[ticketsLength] = receiverRewards1[ticketsLength].add(
        ETH_REWARD.mul(50).div(500).mul(MAX_TICKETS.sub(totalTickets)).div(MAX_TICKETS)
      );
      totalTickets = 0;
    }
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      receiverRewards2[ticketsLength] = receiverRewards2[ticketsLength].add(
        ETH_REWARD.mul(25).div(125).mul(MAX_TICKETS.sub(totalTickets)).div(MAX_TICKETS)
      );
      totalTickets = 0;
    }
  ...
  it("staker can submit compressed tickets after switch with non zero rewards and extra rewards enabled", async function () {
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      for (let j = 0; j < ticketsLength; j++) {
        ...
        extraRewards1[j] = extraRewards1[j].add(receiverRewardPerEpoch.mul(ticketsByEpoch[i][j]).div(MAX_TICKETS));
      }
      if (!receiverRewards1[ticketsLength]) receiverRewards1[ticketsLength] = BN.from(0);
      if (!extraRewards1[ticketsLength]) extraRewards1[ticketsLength] = BN.from(0);
      receiverRewards1[ticketsLength] = receiverRewards1[ticketsLength].add(
        ETH_REWARD.mul(50).div(500).mul(MAX_TICKETS.sub(totalTickets)).div(MAX_TICKETS)
      );
      extraRewards1[ticketsLength] = extraRewards1[ticketsLength].add(
        receiverRewardPerEpoch.mul(MAX_TICKETS.sub(totalTickets)).div(MAX_TICKETS)
      );
      totalTickets = 0;
    }
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      for (let j = 0; j < ticketsLength; j++) {
        ...
        extraRewards2[j] = extraRewards2[j].add(receiverRewardPerEpoch.mul(ticketsByEpoch[i][j]).div(MAX_TICKETS));
      }
      if (!receiverRewards2[ticketsLength]) receiverRewards2[ticketsLength] = BN.from(0);
      if (!extraRewards2[ticketsLength]) extraRewards2[ticketsLength] = BN.from(0);
      receiverRewards2[ticketsLength] = receiverRewards2[ticketsLength].add(
        ETH_REWARD.mul(25).div(125).mul(MAX_TICKETS.sub(totalTickets)).div(MAX_TICKETS)
      );
  ...
  it("staker cannot submit compressed tickets multiple times for the same epoch", async function () {
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      if (!receiverRewards2[ticketsLength]) receiverRewards2[ticketsLength] = BN.from(0);
      receiverRewards2[ticketsLength] = receiverRewards2[ticketsLength].add(
        ETH_REWARD.mul(25).mul(MAX_TICKETS.sub(totalTickets)).div(125).div(MAX_TICKETS)
      );
      totalTickets = 0;
    }
    ...
    await expect(clusterRewards.connect(signers[5])["issueTickets(bytes)"](rawTicketInfo)).to.be.revertedWith(
      "CRW:IPRT-Tickets already issued"
    );
  });
  ...
  it("staker cannot submit signed tickets for future epochs", async function () {
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      if (!receiverRewards1[ticketsLength]) receiverRewards1[ticketsLength] = BN.from(0);
      receiverRewards1[ticketsLength] = receiverRewards1[ticketsLength].add(
        ETH_REWARD.mul(50).mul(MAX_TICKETS.sub(totalTickets)).div(500).div(MAX_TICKETS)
      );
      totalTickets = 0;
    }
  ...
  it("staker cannot submit partial tickets", async function () {
    ...
    await expect(clusterRewards.connect(signers[5])["issueTickets(bytes)"](rawTicketInfo)).to.be.revertedWith(
      "CR:IPTI-invalid ticket info encoding"
    );
  });
  ...
  it("staker cannot submit excess tickets", async function () {
    ...
    await expect(clusterRewards.connect(signers[5])["issueTickets(bytes)"](rawTicketInfo)).to.be.revertedWith(
      "CRW:IPRT-Total ticket count invalid"
    );
  });
  ...
  it("staker cannot submit signed tickets over maximum", async function () {
    ...
    await expect(clusterRewards.connect(signers[15])["issueTickets(bytes)"](rawTicketInfo)).to.be.revertedWith(
      "CR:IPTI-invalid ticket info encoding"
    );
  });
});
...
describe("ClusterRewards: Add Receiver extra payment", function () {
  ...
  it("_increaseReceiverBalance: can be only called by specific role", async () => {
    await expect(clusterRewards.connect(signer)._increaseReceiverBalance(signer.getAddress(), 50)).to.be.revertedWith(
  ...
  it("_setReceiverRewardPerEpoch: can be only called by specific role", async () => {
    await expect(clusterRewards.connect(signer)._setReceiverRewardPerEpoch(signer.getAddress(), 50)).to.be.revertedWith(
      `AccessControl: account ${(
        await signer.getAddress()
      ).toLowerCase()} is missing role ${await clusterRewards.RECEIVER_PAYMENTS_MANAGER()}`
    );
  ...
  it("only admin can grant RECEIVER_PAYMENTS_MANAGER", async () => {
    await expect(
      clusterRewards.connect(signer).grantRole(await clusterRewards.RECEIVER_PAYMENTS_MANAGER(), signer.getAddress())
    ).to.be.revertedWith(
