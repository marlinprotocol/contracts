...
import {
  getRandomNumber,
  randomlyDivideInXPieces,
} from "../../benchmarks/helpers/util";
...
const tickets = [
  ...
  MAX_TICKETS.sub(
    MAX_TICKETS.mul(10)
      .div(100)
      .add(MAX_TICKETS.mul(20).div(100))
      .add(MAX_TICKETS.mul(30).div(100))
      .add(MAX_TICKETS.mul(15).div(100))
  ),
];
...
describe("ClusterRewards deploy and init", function () {
  ...
  it("deploys as proxy and initializes", async function () {
    ...
    expect(
      await clusterRewards.hasRole(
        await clusterRewards.DEFAULT_ADMIN_ROLE(),
        addrs[0]
      )
    ).to.be.true;
    expect(
      await clusterRewards.hasRole(
        await clusterRewards.CLAIMER_ROLE(),
        addrs[1]
      )
    ).to.be.true;
  ...
  it("upgrades", async function () {
    ...
    await upgrades.upgradeProxy(clusterRewards.address, ClusterRewards, {
      kind: "uups",
      constructorArgs: [],
    });
  ...
  it("does not upgrade without admin", async function () {
    ...
    await expect(
      upgrades.upgradeProxy(
        ...
        ClusterRewards.connect(signers[1]),
        {
          kind: "uups",
          constructorArgs: [],
        }
...
testERC165(
  ...
  async function (signers: Signer[], addrs: string[]) {
    ...
    let clusterRewards = getClusterRewards(
...
describe("ClusterRewards add network", function () {
  ...
  before(async function () {
    ...
    let receiverStaking = await deployMockContract(
      signers[0],
      ReceiverStaking.interface.format()
    );
  ...
  it("admin can add network", async function () {
    ...
    let clusterSelector = await deployMockContract(
      signers[0],
    ...
    await clusterRewards.addNetwork(
    ...
    expect(
      await clusterRewards.rewardWeight(ethers.utils.id("POLYGON"))
    ).to.equal(400);
    expect(
      await clusterRewards.clusterSelectors(ethers.utils.id("POLYGON"))
    ).to.equal(clusterSelector.address);
    expect(await clusterRewards.totalRewardWeight()).to.equal(
      TOTALWEIGHT + 400
    );
  });
  ...
  it("admin cannot add existing network", async function () {
    ...
    await expect(
      clusterRewards.addNetwork(
        ethers.utils.id("ETH"),
        400,
        clusterSelector.address
      )
    ).to.be.revertedWith("CRW:AN-Network already exists");
  });
  ...
  it("admin cannot add network with invalid cluster selector", async function () {
    ...
    ).to.be.revertedWith("CRW:AN-ClusterSelector must exist");
  });
  ...
  it("admin cannot add network if start time does not match", async function () {
    ...
    ).to.be.revertedWith("CRW:AN-start time inconsistent");
  });
  ...
  it("admin cannot add network if epoch length does not match", async function () {
    ...
    ).to.be.revertedWith("CRW:AN-epoch length inconsistent");
  });
  ...
  it("non admin cannot add network", async function () {
    ...
    await expect(
      clusterRewards
        .connect(signers[1])
        .addNetwork(ethers.utils.id("POLYGON"), 400, clusterSelector.address)
    ).to.be.revertedWith("only admin");
  });
});
...
describe("ClusterRewards cluster selector", function () {
  ...
  it("admin can update cluster selector", async function () {
    ...
    await clusterRewards.updateNetwork(
      ethers.utils.id("ETH"),
      ETHWEIGHT,
      clusterSelector.address
    );

    expect(await clusterRewards.rewardWeight(ethers.utils.id("ETH"))).to.equal(
      ETHWEIGHT
    );
    expect(
      await clusterRewards.clusterSelectors(ethers.utils.id("ETH"))
    ).to.equal(clusterSelector.address);
    expect(await clusterRewards.totalRewardWeight()).to.equal(TOTALWEIGHT);
  });
  ...
  it("admin cannot update cluster selector to zero", async function () {
    await expect(
      clusterRewards.updateNetwork(
        ethers.utils.id("ETH"),
        ETHWEIGHT,
        ethers.constants.AddressZero
      )
    ).to.be.revertedWith("CRW:UN-ClusterSelector must exist");
  });
  ...
  it("admin cannot update cluster selector if start time does not match", async function () {
    ...
    ).to.be.revertedWith("CRW:UN-start time inconsistent");
  });
  ...
  it("admin cannot update cluster selector if epoch length does not match", async function () {
    ...
    ).to.be.revertedWith("CRW:UN-epoch length inconsistent");
  });
  ...
  it("admin cannot update cluster selector and weight for non existent network", async function () {
    ...
    ).to.be.revertedWith("CRW:UN-Network doesnt exist");
  });
  ...
  it("non admin cannot update weight", async function () {
    await expect(
      clusterRewards
        .connect(signers[1])
        .updateNetwork(ethers.utils.id("ETH"), 400, addrs[11])
    ).to.be.revertedWith("only admin");
  });
  ...
  it("non admin cannot update cluster selector and weight", async function () {
    ...
    await expect(
      clusterRewards
        .connect(signers[1])
        .updateNetwork(ethers.utils.id("ETH"), 400, clusterSelector.address)
    ).to.be.revertedWith("only admin");
  });
});
...
describe("ClusterRewards remove network", function () {
  ...
  it("admin can remove network", async function () {
    ...
    expect(await clusterRewards.rewardWeight(ethers.utils.id("DOT"))).to.equal(
      0
    );
    expect(
      await clusterRewards.clusterSelectors(ethers.utils.id("DOT"))
    ).to.equal(ethers.constants.AddressZero);
    expect(await clusterRewards.totalRewardWeight()).to.equal(
      TOTALWEIGHT - DOTWEIGHT
    );
  });

  it("admin cannot remove non-existing network", async function () {
    await expect(clusterRewards.removeNetwork(ethers.utils.id("POLYGON"))).to.be
      .reverted;
  });

  it("non admin cannot remove network", async function () {
    await expect(
      clusterRewards.connect(signers[1]).removeNetwork(ethers.utils.id("DOT"))
    ).to.be.reverted;
  });
});
...
describe("ClusterRewards update global vars", function () {
  ...
  it("non admin cannot update receiver staking", async function () {
    await expect(
      clusterRewards.connect(signers[1]).updateReceiverStaking(addrs[10])
    ).to.be.reverted;
  });
  ...
  it("non admin cannot update reward per epoch", async function () {
    await expect(clusterRewards.connect(signers[1]).changeRewardPerEpoch(30000))
      .to.be.reverted;
  });
});
...
describe("ClusterRewards submit tickets", function () {
  ...
  before(async function () {
    ...
    const ReceiverStaking = await ethers.getContractFactory("ReceiverStaking");
    receiverStaking = await deployMockContract(
      signers[0],
      ReceiverStaking.interface.format()
    );
    await receiverStaking.mock.START_TIME.returns(startTime);
    await receiverStaking.mock.EPOCH_LENGTH.returns(receiverEpochLength);

    const ClusterSelector = await ethers.getContractFactory("ClusterSelector");
    ethSelector = await deployMockContract(
      signers[0],
      ClusterSelector.interface.format()
    );
  ...
  it("staker can submit tickets", async function () {
    ...
    await receiverStaking.mock.balanceOfSignerAt
      .withArgs(addrs[5], epochToUse)
      .returns(receiverStakeAmount, addrs[4]);
    await receiverStaking.mock.getEpochInfo.reverts();
    await receiverStaking.mock.getEpochInfo
      .withArgs(2)
      .returns(totalEpochStake, 5);
    await ethSelector.mock.getClusters.returns([
      addrs[31],
      addrs[32],
      addrs[33],
      addrs[34],
    ...
    let receiverRewards1 = tickets.map((e) =>
      ETH_REWARD.mul(receiverStakeAmount)
        .mul(e)
        .div(totalEpochStake)
        .div(MAX_TICKETS)
    );

    expect(await clusterRewards.isTicketsIssued(addrs[4], epochToUse)).to.be
      .true;
    expect(await clusterRewards.isTicketsIssued(addrs[5], epochToUse)).to.be
      .false;
    expect(await clusterRewards.clusterRewards(addrs[31])).to.closeTo(
      receiverRewards1[0],
      1
    );
    expect(await clusterRewards.clusterRewards(addrs[32])).to.closeTo(
      receiverRewards1[1],
      1
    );
    expect(await clusterRewards.clusterRewards(addrs[33])).to.closeTo(
      receiverRewards1[2],
      1
    );
    expect(await clusterRewards.clusterRewards(addrs[34])).to.closeTo(
      receiverRewards1[3],
      1
    );
    expect(await clusterRewards.clusterRewards(addrs[35])).to.closeTo(
      receiverRewards1[4],
      1
    );
    ...
    await receiverStaking.mock.balanceOfSignerAt
      .withArgs(addrs[6], 2)
      .returns(anotherReceiverStake, addrs[7]);
    await clusterRewards
      .connect(signers[6])
      ["issueTickets(bytes32,uint24,uint16[])"](
        ETHHASH,
    ...
    let receiverRewards2 = tickets.map((e) =>
      ETH_REWARD.mul(anotherReceiverStake)
        .mul(e)
        .div(totalEpochStake)
        .div(MAX_TICKETS)
    );
    ...
    expect(await clusterRewards.clusterRewards(addrs[31])).to.closeTo(
      receiverRewards1[0].add(receiverRewards2[0]),
      1
    );
    expect(await clusterRewards.clusterRewards(addrs[32])).to.closeTo(
      receiverRewards1[1].add(receiverRewards2[1]),
      1
    );
    expect(await clusterRewards.clusterRewards(addrs[33])).to.closeTo(
      receiverRewards1[2].add(receiverRewards2[2]),
      1
    );
    expect(await clusterRewards.clusterRewards(addrs[34])).to.closeTo(
      receiverRewards1[3].add(receiverRewards2[3]),
      1
    );
    expect(await clusterRewards.clusterRewards(addrs[35])).to.closeTo(
      receiverRewards1[4].add(receiverRewards2[4]),
      1
    );
  });
  ...
  it("staker can submit tickets after switch with non zero rewards", async function () {
    ...
    await receiverStaking.mock.balanceOfSignerAt.reverts();
    await receiverStaking.mock.balanceOfSignerAt
      .withArgs(addrs[5], epochWithRewards)
      .returns(50, addrs[4]);
    await receiverStaking.mock.getEpochInfo.reverts();
    await receiverStaking.mock.getEpochInfo
      .withArgs(epochWithRewards)
      .returns(500, epochWithRewards + 2);
    await ethSelector.mock.getClusters.returns([
      addrs[31],
      addrs[32],
      addrs[33],
    ...
    let receiverRewards1 = tickets.map((e) =>
      ETH_REWARD.mul(50).mul(e).div(500).div(MAX_TICKETS)
    );
    ...
    expect(await clusterRewards.isTicketsIssued(addrs[4], epochWithRewards)).to
      .be.true;
    expect(await clusterRewards.isTicketsIssued(addrs[5], epochWithRewards)).to
      .be.false;
    ...
    await receiverStaking.mock.balanceOfSignerAt.reverts();
    await receiverStaking.mock.balanceOfSignerAt
      .withArgs(addrs[5], epochWithRewards + 2)
      .returns(25, addrs[4]);
    await receiverStaking.mock.getEpochInfo.reverts();
    await receiverStaking.mock.getEpochInfo
      .withArgs(epochWithRewards + 2)
      .returns(125, epochWithRewards + 3);
    await ethSelector.mock.getClusters.returns([
      addrs[35],
      addrs[34],
      addrs[33],
    ...
    let receiverRewards2 = tickets.map((e) =>
      ETH_REWARD.mul(25).mul(e).div(125).div(MAX_TICKETS)
    );
    ...
    expect(await clusterRewards.isTicketsIssued(addrs[4], epochWithRewards + 2))
      .to.be.true;
    expect(await clusterRewards.isTicketsIssued(addrs[5], epochWithRewards + 2))
      .to.be.false;
    expect(await clusterRewards.clusterRewards(addrs[31])).to.be.closeTo(
      receiverRewards1[0].add(receiverRewards2[4]),
      2
    );
    expect(await clusterRewards.clusterRewards(addrs[32])).to.be.closeTo(
      receiverRewards1[1].add(receiverRewards2[3]),
      2
    );
    expect(await clusterRewards.clusterRewards(addrs[33])).to.be.closeTo(
      receiverRewards1[2].add(receiverRewards2[2]),
    ...
    expect(await clusterRewards.clusterRewards(addrs[34])).to.be.closeTo(
      receiverRewards1[3].add(receiverRewards2[1]),
      2
    );
    expect(await clusterRewards.clusterRewards(addrs[35])).to.be.closeTo(
      receiverRewards1[4].add(receiverRewards2[0]),
      2
    );
  });
  ...
  it("staker cannot submit tickets multiple times for the same epoch", async function () {
    ...
    ).to.be.revertedWith("CRW:IPRT-Tickets already issued");
  });

  it("staker can submit tickets with extra rewards enabled", async function () {
    const epochWithRewards = (33 * 86400) / 900 + 2;
    await receiverStaking.mock.balanceOfSignerAt.reverts();

    await receiverStaking.mock.balanceOfAt
      .withArgs(addrs[4], epochWithRewards)
      .returns(50);
    await receiverStaking.mock.signerToStaker
      .withArgs(addrs[5])
      .returns(addrs[4]);

    const mockRewardDelegators = signers[49]; // any random address can be used
    const receiverRewardPerEpoch = BN.from(3456);
    const receiverBalance = BN.from(100000);
    await clusterRewards
      .connect(signers[0])
      .grantRole(
        await clusterRewards.RECEIVER_PAYMENTS_MANAGER(),
        mockRewardDelegators.getAddress()
      );
    await clusterRewards
      .connect(mockRewardDelegators)
      ._increaseReceiverBalance(signers[4].getAddress(), receiverBalance);
    await clusterRewards
      .connect(mockRewardDelegators)
      ._setReceiverRewardPerEpoch(
        signers[4].getAddress(),
        receiverRewardPerEpoch
      );
    ...
    let extraRewards1 = tickets.map((e) =>
      receiverRewardPerEpoch.mul(e).div(MAX_TICKETS)
    );
  ...
  it("staker cannot submit excess tickets", async function () {
    ...
    await receiverStaking.mock.getEpochInfo
      .withArgs(epochWithRewards)
      .returns(500, epochWithRewards + 3);
    await ethSelector.mock.getClusters.returns([
      addrs[31],
      addrs[32],
      addrs[33],
  ...
  it("staker cannot submit tickets over maximum", async function () {
    ...
    await expect(
      clusterRewards
        .connect(signers[5])
        ["issueTickets(bytes32,uint24,uint16[])"](ETHHASH, epochWithRewards, [
          MAX_TICKETS_1_pc.mul(10),
          MAX_TICKETS_1_pc.mul(20),
          MAX_TICKETS_1_pc.mul(30),
          MAX_TICKETS.add(1),
...
describe("ClusterRewards submit compressed tickets", function () {
  ...
  it("staker can submit compressed tickets for 1 epoch", async function () {
    ...
    await receiverStaking.mock.totalSupplyAtRanged.revertsWithReason(
      "unexpected query for total balance"
    );
    await receiverStaking.mock.totalSupplyAtRanged
      .withArgs(2, 1)
      .returns([500]);
    await receiverStaking.mock.balanceOfSignerAtRanged.revertsWithReason(
      "unexpected query for signer balance"
    );
    await receiverStaking.mock.balanceOfSignerAtRanged
      .withArgs(addrs[5], 2, 1)
      .returns([50], addrs[4]);
    await ethSelector.mock.getClustersRanged.returns([
      [addrs[31], addrs[32], addrs[33], addrs[34], addrs[35]],
    ]);
    ...
    for (let i = 0; i < 1 * ticketsLength; i++) {
      ...
      rawTicketInfo =
        rawTicketInfo + tickets[j][k].toString(16).padStart(4, "0");
    }
    let lastTicket = MAX_TICKETS.sub(
      tickets[0].reduce((prev, curr) => prev.add(curr), BN.from(0))
    );
    let receiverRewards1 = tickets[0].map((e) =>
      ETH_REWARD.mul(50).mul(e).div(500).div(MAX_TICKETS)
    );
    let lastReward = ETH_REWARD.mul(50)
      .mul(lastTicket)
      .div(500)
      .div(MAX_TICKETS);
    ...
    await expect(
      clusterRewards.connect(signers[5])["issueTickets(bytes)"](rawTicketInfo)
    ).to.emit(clusterRewards, "TicketsIssued");
    ...
    expect(await clusterRewards.clusterRewards(addrs[31])).to.equal(
      receiverRewards1[0]
    );
    expect(await clusterRewards.clusterRewards(addrs[32])).to.equal(
      receiverRewards1[1]
    );
    expect(await clusterRewards.clusterRewards(addrs[33])).to.equal(
      receiverRewards1[2]
    );
    expect(await clusterRewards.clusterRewards(addrs[34])).to.equal(
      receiverRewards1[3]
    );
    expect(await clusterRewards.clusterRewards(addrs[35])).to.closeTo(
      lastReward,
  ...
  it("staker can submit compressed tickets after switch with non zero rewards", async function () {
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      for (let j = 0; j < ticketsLength; j++) {
        ticketsByEpoch[i][j] = parseInt(
          ((Math.random() * 2) ^ (16 / (ticketsLength + 1))) + ""
        );
        totalTickets += ticketsByEpoch[i][j];
        rawTicketInfo =
          rawTicketInfo + ticketsByEpoch[i][j].toString(16).padStart(4, "0");
        if (!receiverRewards1[j]) receiverRewards1[j] = BN.from(0);
        receiverRewards1[j] = receiverRewards1[j].add(
          ETH_REWARD.mul(50).mul(ticketsByEpoch[i][j]).div(500).div(MAX_TICKETS)
        );
      }
      if (!receiverRewards1[ticketsLength])
        receiverRewards1[ticketsLength] = BN.from(0);
      receiverRewards1[ticketsLength] = receiverRewards1[ticketsLength].add(
        ETH_REWARD.mul(50)
          .div(500)
          .mul(MAX_TICKETS.sub(totalTickets))
          .div(MAX_TICKETS)
      );
      totalTickets = 0;
    }

    await receiverStaking.mock.getCurrentEpoch.returns(
    ...
    await receiverStaking.mock.totalSupplyAtRanged
      .withArgs(startEpoch, numberOfEpochs)
      .returns(Array(numberOfEpochs).fill(500));
    await receiverStaking.mock.balanceOfSignerAtRanged.revertsWithReason(
      "unexpected query for signer balance"
    );
    await receiverStaking.mock.balanceOfSignerAtRanged
      .withArgs(addrs[5], startEpoch, numberOfEpochs)
      .returns(Array(numberOfEpochs).fill(50), addrs[4]);
    await ethSelector.mock.getClustersRanged.returns(
      Array(numberOfEpochs).fill([
        addrs[31],
        addrs[32],
        addrs[33],
        addrs[34],
    ...
    await clusterRewards
      .connect(signers[5])
      ["issueTickets(bytes)"](rawTicketInfo);

    for (let i = 0; i < numberOfEpochs; i++) {
      expect(await clusterRewards.isTicketsIssued(addrs[4], startEpoch + i)).to
        .be.true;
      expect(await clusterRewards.isTicketsIssued(addrs[5], startEpoch + i)).to
        .be.false;
      expect(await clusterRewards.clusterRewards(addrs[31])).to.be.closeTo(
        receiverRewards1[0],
        1
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      for (let j = 0; j < ticketsLength; j++) {
        ...
        receiverRewards2[j] = receiverRewards2[j].add(
          ETH_REWARD.mul(25).mul(ticketsByEpoch[i][j]).div(125).div(MAX_TICKETS)
        );
      }
      if (!receiverRewards2[ticketsLength])
        receiverRewards2[ticketsLength] = BN.from(0);
      receiverRewards2[ticketsLength] = receiverRewards2[ticketsLength].add(
        ETH_REWARD.mul(25)
          .div(125)
          .mul(MAX_TICKETS.sub(totalTickets))
          .div(MAX_TICKETS)
    ...
    await receiverStaking.mock.totalSupplyAtRanged
      .withArgs(startEpoch, numberOfEpochs)
      .returns(Array(numberOfEpochs).fill(125));
    await receiverStaking.mock.balanceOfSignerAtRanged.revertsWithReason(
      "unexpected query for signer balance"
    );
    await receiverStaking.mock.balanceOfSignerAtRanged
      .withArgs(addrs[5], startEpoch, numberOfEpochs)
      .returns(Array(numberOfEpochs).fill(25), addrs[4]);
    await ethSelector.mock.getClustersRanged.returns(
      Array(numberOfEpochs).fill([
        addrs[35],
        addrs[34],
  ...
  it("staker can submit compressed tickets after switch with non zero rewards and extra rewards enabled", async function () {
    const epochWithRewards = (33 * 86400) / 900 + 2;
    const numberOfEpochs = 10;
    let receiverRewards1: BN[] = [];
    let extraRewards1: BN[] = [];
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      for (let j = 0; j < ticketsLength; j++) {
        ...
        if (!extraRewards1[j]) extraRewards1[j] = BN.from(0);
        extraRewards1[j] = extraRewards1[j].add(
          receiverRewardPerEpoch.mul(ticketsByEpoch[i][j]).div(MAX_TICKETS)
        );
      }
      if (!receiverRewards1[ticketsLength])
        receiverRewards1[ticketsLength] = BN.from(0);
      if (!extraRewards1[ticketsLength])
        extraRewards1[ticketsLength] = BN.from(0);
      receiverRewards1[ticketsLength] = receiverRewards1[ticketsLength].add(
        ETH_REWARD.mul(50)
          .div(500)
          .mul(MAX_TICKETS.sub(totalTickets))
      ...
      extraRewards1[ticketsLength] = extraRewards1[ticketsLength].add(
        receiverRewardPerEpoch
          .mul(MAX_TICKETS.sub(totalTickets))
          .div(MAX_TICKETS)
      );
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      for (let j = 0; j < ticketsLength; j++) {
        ...
        if (!extraRewards2[j]) extraRewards2[j] = BN.from(0);
        extraRewards2[j] = extraRewards2[j].add(
          receiverRewardPerEpoch.mul(ticketsByEpoch[i][j]).div(MAX_TICKETS)
        );
      }
      if (!receiverRewards2[ticketsLength])
        receiverRewards2[ticketsLength] = BN.from(0);
      if (!extraRewards2[ticketsLength])
        extraRewards2[ticketsLength] = BN.from(0);
      receiverRewards2[ticketsLength] = receiverRewards2[ticketsLength].add(
        ETH_REWARD.mul(25)
          .div(125)
          .mul(MAX_TICKETS.sub(totalTickets))
      ...
      extraRewards2[ticketsLength] = extraRewards2[ticketsLength].add(
        receiverRewardPerEpoch
          .mul(MAX_TICKETS.sub(totalTickets))
          .div(MAX_TICKETS)
      );
  ...
  it("staker cannot submit compressed tickets multiple times for the same epoch", async function () {
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      for (let j = 0; j < ticketsLength; j++) {
        ...
        if (!receiverRewards1[j]) receiverRewards1[j] = BN.from(0);
        receiverRewards1[j] = receiverRewards1[j].add(
          ETH_REWARD.mul(50).div(500).mul(ticketsByEpoch[i][j]).div(MAX_TICKETS)
        );
      }
      if (!receiverRewards1[ticketsLength])
        receiverRewards1[ticketsLength] = BN.from(0);
  ...
  it("staker cannot submit signed tickets for future epochs", async function () {
    ...
    await expect(
      clusterRewards.connect(signers[5])["issueTickets(bytes)"](rawTicketInfo)
    ).to.be.revertedWith("CRW:ITC-Epochs not completed");
  });
  ...
  it("staker cannot submit partial tickets", async function () {
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ...
      for (let j = 0; j < ticketsLength; j++) {
        ...
        if (FuzzedNumber.randomInRange(0, 100).toNumber() > 20) {
          // drop in 20% of cases
          rawTicketInfo =
            rawTicketInfo + ticketsByEpoch[i][j].toString(16).padStart(4, "0");
        }
      ...
      if ((rawTicketInfo.length - 2 - 8 - 64) % 16 == 0) {
        rawTicketInfo =
          rawTicketInfo +
          parseInt(((Math.random() * 2) ^ (16 / (ticketsLength + 1))) + "")
            .toString(16)
            .padStart(4, "0");
      }
    }
    ...
    await expect(
      clusterRewards.connect(signers[5])["issueTickets(bytes)"](rawTicketInfo)
    ).to.be.revertedWith("CR:IPTI-invalid ticket info encoding");
  });
  ...
  it("staker cannot submit excess tickets", async function () {
    ...
    for (let i = 0; i < numberOfEpochs; i++) {
      ticketsByEpoch[i] = [];
      ticketsByEpoch[i] = randomlyDivideInXPieces(
        MAX_TICKETS.add(1),
        ticketsLength
      ).map((e) => e.toNumber());
      for (let j = 0; j < ticketsLength; j++) {
  ...
  it("staker cannot submit signed tickets over maximum", async function () {
    ...
    await expect(
      clusterRewards.connect(signers[15])["issueTickets(bytes)"](rawTicketInfo)
    ).to.be.revertedWith("CR:IPTI-invalid ticket info encoding");
  });
});
...
describe("ClusterRewards claim rewards", function () {
  ...
  it("claimer can claim rewards", async function () {
    ...
    expect(
      await clusterRewards.connect(signers[1]).callStatic.claimReward(addrs[33])
    ).to.be.closeTo(receiverRewards1[2].sub(1), 1);
    ...
    expect(
      await clusterRewards.connect(signers[1]).callStatic.claimReward(addrs[31])
    ).to.be.closeTo(receiverRewards1[0].sub(1), 1);
    ...
    expect(
      await clusterRewards.connect(signers[1]).callStatic.claimReward(addrs[35])
    ).to.be.closeTo(receiverRewards1[4].sub(1), 1);
    ...
    expect(
      await clusterRewards.connect(signers[1]).callStatic.claimReward(addrs[34])
    ).to.be.closeTo(receiverRewards1[3].sub(1), 1);
    ...
    expect(
      await clusterRewards.connect(signers[1]).callStatic.claimReward(addrs[32])
    ).to.be.closeTo(receiverRewards1[1].sub(1), 1);
    ...
    expect(
      await clusterRewards.connect(signers[1]).callStatic.claimReward(addrs[36])
    ).to.equal(0);
  ...
  it("non claimer cannot claim rewards", async function () {
    ...
    await expect(clusterRewards.claimReward(addrs[33])).to.be.revertedWith(
      `AccessControl: account ${addrs[0].toLowerCase()} is missing role ${await clusterRewards.CLAIMER_ROLE()}`
    );
  });
});
...
describe("ClusterRewards: Add Receiver extra payment", function () {
  ...
  before(async function () {
    ...
    mockToken = await upgrades.deployProxy(Pond, ["Marlin", "POND"], {
      kind: "uups",
    });
    ...
    await receiverStaking.mock.signerToStaker
      ...
      .returns(await staker.getAddress());
    await receiverStaking.mock.stakerToSigner
      .withArgs(await staker.getAddress())
      .returns(await signer.getAddress());
  ...
  it("_increaseReceiverBalance: can be only called by specific role", async () => {
    await expect(
      clusterRewards
        .connect(signer)
        ._increaseReceiverBalance(signer.getAddress(), 50)
    ).to.be.revertedWith(
  ...
  it("_setReceiverRewardPerEpoch: can be only called by specific role", async () => {
    await expect(
      clusterRewards
        .connect(signer)
        ._setReceiverRewardPerEpoch(signer.getAddress(), 50)
    ).to.be.revertedWith(
      `AccessControl: account ${(
        await signer.getAddress()
      ).toLowerCase()} is missing role ${await clusterRewards.RECEIVER_PAYMENTS_MANAGER()}`
  ...
  it("only admin can grant RECEIVER_PAYMENTS_MANAGER", async () => {
    await expect(
      ...
      `AccessControl: account ${(
        await signer.getAddress()
      ).toLowerCase()} is missing role ${await clusterRewards.DEFAULT_ADMIN_ROLE()}`
    );
  ...
  it("Check increase balance", async () => {
    ...
    const balanceBefore = (
      await clusterRewards.receiverRewardPayment(signer.getAddress())
    )[0];
    await clusterRewards
      .connect(signer)
      ._increaseReceiverBalance(signer.getAddress(), increaseByAmount);
    const balanceAfter = (
      await clusterRewards.receiverRewardPayment(signer.getAddress())
    )[0];
  ...
  it("Check reward per epoch setting", async () => {
    ...
    await clusterRewards
      ...
      ._setReceiverRewardPerEpoch(
        staker.getAddress(),
        receiverExtraRewardPerEpoch
      );
    expect(
      (await clusterRewards.receiverRewardPayment(staker.getAddress()))[1]
    ).eq(receiverExtraRewardPerEpoch);
  });
  ...
  it("Reward Checking after receiver adds extra tokens", async () => {
    ...
    await clusterRewards
      .connect(mockRewardDelegators)
      ._increaseReceiverBalance(staker.getAddress(), receiverBalance);

    // reward will set only by signer, hence we are using signer address here
    await clusterRewards
      .connect(mockRewardDelegators)
      ._setReceiverRewardPerEpoch(staker.getAddress(), receiverRewardPerEpoch);
