...
describe("ClusterRewards update global vars", function () {
  ...
  it("non admin cannot update reward wait time", async function () {
    await expect(clusterRewards.connect(signers[1]).updateRewardWaitTime(30000))
      .to.be.reverted;
  });
});
...
describe("ClusterRewards feed rewards", function () {
  ...
  const FEEDER_REWARD_1_pc = FEED_REWARD.mul(ETHWEIGHT)
    .div(TOTALWEIGHT)
    .div(100)
    .mul(24 * 60 * 60)
    .div(900);
  ...
  before(async function () {
    ...
    await clusterRewards.grantRole(
      await clusterRewards.FEEDER_ROLE(),
      addrs[2]
    );
  ...
  it("feeder can feed rewards before start time", async function () {
    await clusterRewards
      .connect(signers[2])
      .feed(ETHHASH, [addrs[21], addrs[22]], [e16.mul(10), e16.mul(50)], 1);

    expect(await clusterRewards.clusterRewards(addrs[21])).to.equal(
      FEEDER_REWARD_1_pc.mul(10)
    );
    expect(await clusterRewards.clusterRewards(addrs[22])).to.equal(
      FEEDER_REWARD_1_pc.mul(50)
    );
    expect(await clusterRewards.rewardDistributedPerEpoch(1)).to.equal(
      FEEDER_REWARD_1_pc.mul(60)
    );
  });
  ...
  it("feeder cannot feed rewards after 1 day after switching time", async function () {
    ...
    await expect(
      clusterRewards
        .connect(signers[2])
        .feed(ETHHASH, [addrs[21], addrs[22]], [e16.mul(10), e16.mul(50)], 1)
    ).to.be.revertedWith("CRW:F-Invalid method");
  });
  ...
  it("feeder cannot feed rewards exceeding total", async function () {
    await expect(
      clusterRewards
        ...
        .feed(
          ETHHASH,
          [addrs[21], addrs[22], addrs[23]],
          [e16.mul(10), e16.mul(50), e16.mul(40).add(e16.div(100))],
          1
        )
    ).to.be.revertedWith(
  ...
  it("feeder can feed rewards in multiple parts", async function () {
    await clusterRewards
      .connect(signers[2])
      .feed(ETHHASH, [addrs[21]], [e16.mul(10)], 1);
    ...
    await clusterRewards
      .connect(signers[2])
      .feed(ETHHASH, [addrs[22]], [e16.mul(50)], 1);
  ...
  it("feeder cannot feed rewards in multiple parts exceeding total", async function () {
    ...
    await expect(
      clusterRewards
        .connect(signers[2])
        .feed(ETHHASH, [addrs[23]], [e16.mul(41)], 1)
    ).to.be.revertedWith(
      "CRW:F-Reward Distributed  cant  be more  than totalRewardPerEpoch"
    );
  });
  ...
  it("feeder cannot feed rewards again before wait time", async function () {
    ...
    await expect(
      clusterRewards
        .connect(signers[2])
        .feed(ETHHASH, [addrs[21], addrs[22]], [e16.mul(10), e16.mul(10)], 2)
    ).to.be.revertedWith(
  ...
  it("feeder can feed rewards again after wait time", async function () {
    ...
    expect(await clusterRewards.rewardDistributedPerEpoch(2)).to.equal(
      FEEDER_REWARD_1_pc.mul(20)
    );
  });
