...
describe("GovernorAlpha", function () {
  ...
  it("deploys with initialization disabled", async function () {
    ...
    let governorAlpha = getGovernorAlpha(
      governorAlphaContract.address,
      signers[0]
    );
    await expect(
      governorAlpha.initialize(timelock.address, mpond.address, addrs[1])
    ).to.be.reverted;
  });

  it("deploys as proxy and initializes", async function () {
    const GovernorAlpha = await ethers.getContractFactory("GovernorAlpha");
    let governorAlphaContract = await upgrades.deployProxy(
      GovernorAlpha,
      [timelock.address, mpond.address, addrs[1]],
      { kind: "uups" }
    );
    let governorAlpha = getGovernorAlpha(
      governorAlphaContract.address,
    ...
    expect(
      await governorAlpha.hasRole(
        await governorAlpha.DEFAULT_ADMIN_ROLE(),
        addrs[0]
      )
    ).to.be.true;
  ...
  it("upgrades", async function () {
    ...
    await upgrades.upgradeProxy(governorAlpha.address, GovernorAlpha, {
      kind: "uups",
    });
  ...
  it("does not upgrade without admin", async function () {
    ...
    await expect(
      upgrades.upgradeProxy(
        ...
        GovernorAlpha.connect(signers[1]),
        { kind: "uups" }
      )
    ).to.be.reverted;
  });
