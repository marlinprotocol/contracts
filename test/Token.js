...
contract("TokenTestContract", function (accounts) {
    var tokenInstance;
    ...
    it("initializes with token", function () {
        ...
        return TokenLogic.at(tokenProxy.address)
          .then(function (instance) {
            ...
            return tokenInstance
              .initialize("Marlin", "POND", tempBridgeAddress)
              .then(function () {
      ...
      it("Checking if deployer is minter and any other account is not a minter and then add new minter", function () {
        var minerRole = web3Utils.keccak256("MINTER_ROLE");
        return tokenInstance.hasRole(minerRole, accounts[0])
          .then(function (data) {
            return data;
          })
          .then(function (isMinter) {
            assert.equal(isMinter, true, "Not minter");
            return tokenInstance.hasRole(minerRole, accounts[1]);
          })
          .then(function (isMinter) {
            assert.equal(isMinter, false, "Minter");
            return tokenInstance.grantRole(minerRole, accounts[1]);
          })
          .then(function (added) {
            //console.log(added.logs);
            assert.equal(added.logs[0].event, "RoleGranted", "Minter not Added");
            return tokenInstance.hasRole(minerRole, accounts[1]);
          })
          .then(function (isMinter) {
            assert.equal(isMinter, true, "Minter role not granted");
            return tokenInstance.revokeRole(minerRole, accounts[1]);
          })
          ...
          .then(function (isMinter) {
            assert.equal(isMinter, false, "Minter not revoked");
          });
      });
      ...
      it("checking approve, decreaseAllowance, increaseAllowance, burnFrom functions", function () {
        ...
        return tokenInstance.approve(accounts[3], 450)   
          ...
          .then(function (burned) {
            ...
            assert.equal(burned.logs[0].event, "Approval", "Error while approving");
            return tokenInstance.allowance(accounts[0], accounts[3]);
          })
          .then(function (allow) {
